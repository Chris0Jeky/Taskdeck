...
The concrete fix is retry ownership: withdrawing consent invalidates an in-flight batch. A later network failure cannot requeue old events, even after a fresh opt-in. Withdrawal clears memory, rotates the session and stops the timer before attempting storage. Blocked storage must not prevent this cleanup. Already transmitted data cannot be recalled. When a browser refuses the consent-preference write, the in-memory choice still applies for the current session and Taskdeck leaves a persistent warning that the saved state may differ after reload. A failed in-flight batch is also discarded rather than re-buffered when server telemetry becomes inactive before the request settles.

Seven Vitest regression tests cover withdrawal, withdrawal/re-consent, uninterrupted-consent retries, transient server-config loss, blocked revocation writes, blocked opt-in writes and blocked reads. Run the existing frontend test command targeting `telemetryStore.consent.spec.ts`, followed by the repository's usual checks. These host tests have not been executed in this environment.
