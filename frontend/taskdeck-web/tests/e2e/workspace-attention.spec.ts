...
test('reminder hours persist across experiences and recover a concurrent settings change', async ({ page, request }) => {
  ...
  await settings.getByLabel('Time zone', { exact: true }).fill('Not/AZone')
  const invalid = page.waitForResponse(response => response.url().endsWith('/workspace-attention') && response.request().method() === 'PUT')
  await settings.getByRole('button', { name: 'Save reminder hours', exact: true }).click()
  expect((await invalid).status()).toBe(400)
  await expect(settings.getByLabel('Time zone', { exact: true })).toHaveValue('Not/AZone')
