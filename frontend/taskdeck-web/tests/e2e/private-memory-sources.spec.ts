...
test('private originals survive correction, archive, layout switches and a downloaded export', async ({ page, request }) => {
  ...
  await page.getByRole('textbox', { name: 'Memory', exact: true }).fill('  Keep this original exactly.\nStill unknown.  ')
  ...
  await page.getByRole('textbox', { name: 'Memory', exact: true }).fill('We learned enough to take one small step.')
