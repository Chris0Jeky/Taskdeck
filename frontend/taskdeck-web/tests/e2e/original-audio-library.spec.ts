...
test('retained originals remain discoverable after question removal and board archival across all experiences', async ({ page, request }) => {
  ...
  await page.getByLabel('Written audio version', { exact: true }).fill('Keep this unfinished original wording.')
  await page.getByLabel('Layer 1 details', { exact: true }).fill('The shared question has changed.')
  await page.getByRole('button', { name: 'Save thinking', exact: true }).click()
  const retainedDraft = page.getByRole('region', { name: 'Retained written audio draft', exact: true })
  await expect(retainedDraft.getByLabel('Earlier written audio draft', { exact: true })).toHaveValue('Keep this unfinished original wording.')
  await expect(retainedDraft).toContainText('Copy it before discarding')
  await expect(page.getByRole('button', { name: 'Save written version', exact: true })).toHaveCount(0)
  await retainedDraft.getByRole('button', { name: 'Discard earlier written draft', exact: true }).click()
  await expect(retainedDraft).toHaveCount(0)
  const changedDeck = await (await request.get(thinking, { headers })).json()
  await assertOk(await request.put(thinking, { headers, data: { expectedRevision: changedDeck.revision, layers: [] } }), 'remove source question')
