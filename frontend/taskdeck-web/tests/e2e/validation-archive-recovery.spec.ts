...
test.describe('TST11-SC-012: Restore archived board', () => {
  test('restored board reappears in board list with preserved state', async ({ page, request }) => {
    ...
    const boardRow = page.locator('.paper-archive__row').filter({ hasText: `Restore Test ${seed}` })
    ...
    await expect(page.locator('.td-toast:visible').getByText('Restored board')).toBeVisible()
