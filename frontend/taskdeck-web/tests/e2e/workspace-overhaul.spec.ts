...
test('answers an insight privately and preserves memory corrections and archive history', async ({ page, request }) => {
  ...
  await page.getByRole('textbox', { name: 'Memory', exact: true }).fill('We tested the layout and chose a smaller next step.')
  await page.getByRole('button', { name: 'Close', exact: true }).click()
  await expect(page.getByRole('dialog', { name: 'Discard memory draft?' })).toBeVisible()
  await page.getByRole('button', { name: 'Keep editing', exact: true }).click()
  await expect(page.getByRole('textbox', { name: 'Memory', exact: true })).toHaveValue('We tested the layout and chose a smaller next step.')
  ...
  await memory.locator('summary').filter({ hasText: 'Revision history' }).click()
  await expect(memory.locator('details').filter({ has: page.locator('summary').filter({ hasText: 'Revision history' }) })).toHaveAttribute('open', '')
...
test('makes comparison and Grove themes usable on desktop and narrow screens', async ({ page, request }) => {
  ...
  await page.getByRole('textbox', { name: 'Memory', exact: true }).fill('Which presentation helps me notice the next useful step?')
