...
<template>
  <div
    ...
    :class="[
      ...
      isSelected ? 'border-[#ff5352] ring-4 ring-[#ff5352]/30 shadow-xl bg-[#ff5352]/10 scale-105' : 'bg-[#1c1b1b] border-[rgba(91,64,62,0.15)]',
  ...
  >
    <button
      ...
      draggable="true"
      class="td-card-drag-handle -mx-2 -mt-1 mb-2 flex min-h-10 w-[calc(100%+1rem)] items-center justify-center gap-2 rounded-md px-3 py-2 text-[#e5e2e1]/60 hover:bg-[#3a3939] hover:text-[#e5e2e1]/70 cursor-grab active:cursor-grabbing"
      title="Drag Card"
    ...
    >
      ...
      <span class="text-[11px] font-semibold uppercase tracking-[0.2em] font-[Space_Grotesk]">Drag card</span>
    </button>

    <!-- Blocked Badge -->
    <div v-if="card.isBlocked" class="mb-2">
      <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-[#ff4d4d]/10 text-[#ff4d4d] text-xs rounded">
    ...
    <!-- Card Title -->
    <h4 class="text-sm font-bold font-[Manrope] text-[#e5e2e1] group-hover:text-[#ffb3ae] min-w-0 break-words mb-2 transition-colors">{{ card.title }}</h4>

    <!-- Card Description (if exists) -->
    <p v-if="card.description" class="text-xs text-[#e5e2e1]/60 mb-2 line-clamp-2">
      {{ card.description }}
    </p>
    ...
    <div v-if="card.labels.length > 0" class="flex flex-wrap gap-1 mb-2">
      <span
        ...
        :key="label.id"
        class="px-2 py-0.5 text-[9px] uppercase font-[Space_Grotesk] tracking-[0.2em] rounded text-white font-medium"
    ...
    <!-- Due Date -->
    <div v-if="card.dueDate" class="flex items-center gap-1 text-xs" :class="isOverdue(card.dueDate) ? 'text-[#ff4d4d]' : 'text-[#e5e2e1]/60'">
