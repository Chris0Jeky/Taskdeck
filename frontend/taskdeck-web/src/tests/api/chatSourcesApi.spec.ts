...
it('uses an encoded source identity and explicit board, version and page', async () => {
  const page = { memoryId: 'memory', revision: 3, items: [], nextOffset: null, nextAfterOrdinal: null }
  http.get.mockResolvedValue({ data: page })
  expect(await chatSourcesApi.list('memory/id', 'board', 3, 10)).toEqual(page)
  expect(http.get).toHaveBeenCalledWith('/llm/chat/context-memory/memory%2Fid/sources', { params: { boardId: 'board', revision: 3, afterOrdinal: 10 } })
})
