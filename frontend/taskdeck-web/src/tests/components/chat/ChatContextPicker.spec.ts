...
describe('explicit chat context', () => {
  ...
  it('retains chosen context across token refresh and associates originals with the memory title', async () => {
    const { wrapper, session } = setup(); await open(wrapper)
    await wrapper.get('select').setValue('c1')
    await wrapper.findAll('input')[1]!.setValue(true)
    const selection = wrapper.emitted('change')?.at(-1)
    expect(wrapper.getComponent(ChatOriginalSourcePicker).props('memoryTitle')).toBe('Private m1')
    session.token = 'refreshed'; await flushPromises()
    expect(wrapper.get('select').element.value).toBe('c1')
    expect(wrapper.emitted('change')?.at(-1)).toEqual(selection)
    session.token = ''; await flushPromises(); expect(wrapper.emitted('change')?.at(-1)).toEqual([null])
  })
})
