...
onMounted(() => {
  permissions.fetchBoardAccess(props.boardId)
})

const accessList = () => permissions.boardAccess.get(props.boardId) ?? []
...
async function handleGrant() {
  ...
  try {
    granting.value = true
    await permissions.grantAccess(props.boardId, {
      userId: newUserId.value.trim(),
      role: newRole.value,
    })
    newUserId.value = ''
    showGrantForm.value = false
  } catch (e) { console.error('Failed to grant access', e) } finally {
    granting.value = false
  }
}

async function handleRoleChange(accessId: string, role: BoardRole) {
  await permissions.updateAccess(props.boardId, accessId, { role })
}

async function handleRevoke(accessId: string) {
  if (confirm('Are you sure you want to revoke this access?')) {
    await permissions.revokeAccess(props.boardId, accessId)
  }
}
</script>
...
<template>
  <div class="td-access-panel">
    <div class="td-access-header">
      <h2 class="td-section-title">Board Members</h2>
    ...
    <div v-if="showGrantForm" class="td-grant-form">
      ...
      <button class="td-btn td-btn--primary td-btn--sm" @click="handleGrant" :disabled="granting">
    ...
    <div v-else class="td-access-list">
      <div v-if="accessList().length === 0" class="td-empty">No access entries found.</div>
      <div v-for="entry in accessList()" :key="entry.id" class="td-access-row">
        ...
        <div class="td-access-controls">
          <select
            :value="entry.role"
            @change="handleRoleChange(entry.id, ($event.target as HTMLSelectElement).value as BoardRole)"
...
<style scoped>
.td-access-panel { background: var(--td-surface-primary); border-radius: var(--td-radius-lg); padding: var(--td-space-6); border: 1px solid var(--td-border-default); }
