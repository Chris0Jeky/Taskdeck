...
- For a helper-created detached worktree, the first agent command is the complete printed `scripts/worktree_guard.ps1` command with pinned Git, followed by the bounded `scripts/git/Initialize-CodexIssueWorktree.ps1` command; it verifies the exact worktree and detached base before `switch -c`, and removes its unused detached worktree on a late collision
