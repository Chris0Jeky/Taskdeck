...
Claude uses these helpers as its primary GitHub operations path because its project MCP configuration intentionally omits GitHub. Codex uses them as a REST/CLI fallback when its configured GitHub MCP is unavailable or quota-limited. Project v2 priority audit needs `read:project`; applying field updates with `Sync-TaskdeckProjectPriority.ps1 -Apply` requires `gh auth refresh -s project`.
