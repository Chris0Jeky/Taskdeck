...
- It rejects repeated or non-advancing cursors, ordinal duplicate item IDs, truncated label/field-value connections, and project changes observed during pagination.
...
- `-Apply` rechecks the project item count and `updatedAt` immediately before writes. Rerun the read-only audit afterward because ProjectV2 edits are not transactional as a batch.
- `-SelfTest` exercises the offline pagination, saturation, identity, cursor, nested-connection, and invalid-issue-label guards without requiring `gh` authentication.
