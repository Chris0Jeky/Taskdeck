...
Security + a11y quality slices (2026-07-02, **2 PRs merged** — `#1263` (`#1241`), `#1265` (`#1218`); full review gate per PR — independent adversarial reviews, all-severity findings fixed or tracked, Gemini/Codex/Copilot bot rounds resolved, fresh green CI + aging):
...
- **WCAG focus-ring contrast sweep:** `#1265` (`#1218`) sweeps every remaining `ring-primary/50` (18 focus rings + the selected-label state ring) to the full-opacity treatment (`#1216` precedent) — rendered indicators lift from a marginal 3.0–3.5:1 to 8.4–10.1:1 on every adjacent surface across all shipped themes — and removes 6 compile-proven dead checkbox `focus:ring-primary` classes (no ring-width utility; the global `*:focus-visible` rule already covers them at 4.52:1). Seeded: `#1266` (pre-existing white ring-offset halo on the selected-label state).
