...
- **Six bounded product/deployment issue slices are merged.** `#1876`/PR `#2016` ships the secret-free ordinary-profile retired-provider diagnostic and explicit-migration compatibility rule; `#1966`/PR `#2017` makes board-scoped Inbox/Review state visible and clearable; `#1938`/PRs `#2023` + `#2033` preserve failed quick-capture drafts and add persistent expandable/copyable error receipts with an explicit Paper dismiss path; `#1949`/PRs `#2027` + `#2032` extend the dead-affordance source guard to enabled-looking native buttons and labelled custom controls without keyboard semantics; `#1967`/PR `#2034` makes proposal-only identity matching case-insensitive without substituting a different action target; and `#1992`/PRs `#2029` + `#2030` correct lowercase installed-PWA and split-container routing for the machine prefixes. The latter four parent issues remain open for their explicitly recorded residual acceptance criteria.
...
- **Three review ceilings produced durable parked lineages, not fix cascades.** `#1967`/PR `#2025` preserves the broader applied-proposal detail lineage at `a70f88ad`; its mixed-case proposal-identity blocker was extracted into the narrower merged PR `#2034`, while the broader detail/history acceptance remains open. For `#2005`, closed PR `#2026` preserves the older head at `ab1db61e` and PR `#2047` carries the line forward at `9ac1fb5f`, but remains Blocked after final-base review confirmed in-flight metadata loss (`#2051`) and stale stranded-proposal reuse (`#2052`). `#1973`/PR `#2031` remains preserved at `ecf44ea2` after its single fix round still left Paper capture history without a full detail surface and allowed a retained proposal hash to escape the archived read-only boundary. Resume from the issue comments and preserved branches, not by reopening or rewriting those PR heads.
