...
- **The private shared instance remains bounded by ADR-0061/`#1772`.** One application instance, one SQLite volume, a few known users, private access, InviteOnly onboarding, exact-image evidence, SignalR/reconnect and durable-reload proof, application-consistent encrypted backup, separate connector-key backup, restore drill, cost owner, and LLM payer/egress disclosure are required. Render split `#1777` remains Blocked on those decisions. No public signup, tenancy, horizontal scale, billing, or SaaS claim is authorized; Azure Student credit is lab/staging leverage only.
