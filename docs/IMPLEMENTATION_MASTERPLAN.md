...
- **`#1513` local review-fix implementation:** the OpenAI, Gemini, Ollama, and outbound-webhook primary handlers set `UseProxy = false`, keeping the configured origin as the `OutboundWebhookConnectCallback` validation target even when ambient/system proxy settings exist. Concrete providers reuse the same startup-resolved localhost policy as selection and transport validation. The four protected clients remove default request loggers, disable distributed-trace header propagation, and use a private HTTP metric scope; Taskdeck's configured OpenTelemetry provider drops marked activities and that scope, including destination dimensions. Enabled Sentry retains its server-side exception tracking but disables its automatic `IHttpClientFactory` handler, preventing an independent trace/baggage, URL-breadcrumb, or failed-request path around those controls. Independently installed process-global listeners remain outside this guarantee. Existing strict localhost opt-ins and automatic-redirect blocking remain unchanged. No proxy-aware outbound mode exists; corporate proxy-only deployments fail closed. This is the existing dedicated connect-callback path, not `EgressEnvelopeHandler` enforcement, and it does not expand redirect or audit behavior.
- **Current evidence boundary:** 73/73 affected API tests pass across registered pipelines, fully decoded provider dispatch bodies, Production policy injection, proxy canaries, delivery behavior, and OpenTelemetry/Sentry boundaries; 151/151 provider and selection-policy compatibility tests pass. The 6-case provider-dispatch lane passed five consecutive repetitions; the enabled-Sentry four-client success/5xx regression passed ten consecutive repetitions after its disabling control failed as expected and now closes its process-global hub under a nonparallel collection. Docs governance, golden-principles governance, GitHub-operations governance, and `git diff --check` also pass. The merged Dockerless Testcontainers repair `#1518` is absorbed; full serialized verification, final security-boundary review, signed publication, Required CI, CodeQL, exact-head re-review, and merge remain outstanding, so this is not yet a shipped delivery entry.

## Delivery update (2026-07-27, test substrate and Windows API diagnostics)

- **Dockerless Testcontainers gate (`#1518`, merged as `ff1ee066`):** the PostgreSQL collection fixture now evaluates `DockerAvailableCheck` before `PostgreSqlBuilder.Build`, restoring the documented graceful-skip behavior on hosts without Docker. Public-constructor and injected-factory regressions prove the unavailable path cannot construct a container, while a timed-out Docker CLI probe is killed as a process tree and boundedly reaped. Local Integration passed 7 / 28 / 0 with no surviving `docker info`; the serialized backend solution passed 7,465 / 33 / 0; exact-head hosted Container Integration passed 35 / 0 / 0. Future fail-closed hosted zero-skip enforcement is tracked as `#1520`.
