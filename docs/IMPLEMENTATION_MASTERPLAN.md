...
- **Project priority completeness (`#1458`):** replace the capped ProjectV2 item sample with cursor-complete, stable-snapshot pagination and fail-closed checks for saturation, item/cursor identity, nested-connection truncation, project/source-plan drift, invalid issue priority labels, typed repository-aware PR references, partial writer failure, and stale post-apply reporting. The authentication-free self-test covers raw REST normalization, same-repository authority, cross-repository rejection, ordered option dispatch, strict fallback, zero-write preflight, partial failure auditing, and verified post-state output; `-Apply` validates every option and rebuilds the complete derivation plan before the first write. Delivery remains data-gated: a complete 1,458-item read found 130 project issues without a single priority label. The typed PR pass ignores misidentified PR #123, then reaches external Issue `dotnet/efcore#35983`, exposed by GitHub as a closing reference on 12 historical Taskdeck Dependabot PRs; the safe default rejects that external authority. No trusted PR plan or `-Apply` exists; #1458 stays open pending truthful project labels, any explicit cross-repository opt-in decision, and a final zero-drift audit.
