...
28. **Docs, auth, tech-debt, testing, and CI hardening wave (2026-04-23, PRs `#936`--`#942`)**: 7 issues across 7 PRs. **Docs**: docs cleanup (`#934`/`#936` -- duplicate `#326` annotation, `TASKDECK_E2E_DB` prefix removal across 5 docs files), UTF-8 mojibake fix (`#929`/`#937` -- 109 double-encoded sequences in STATUS.md). **Auth**: `POST /auth/refresh` endpoint (`#933`/`#939` -- per-user rate limiting, completes FE-20 session extension end-to-end, 10 integration tests). **Tech-debt**: redundant migration no-op (`#932`/`#938`), UserPreferences `INSERT OR IGNORE` upsert race fix (`#931`/`#941` -- 105 lines to 51 lines, 4 integration tests). **Testing**: test count recertification (`#930`/`#940` -- 7,586+ combined: backend 4,979, frontend 2,607). **CI**: 5 pre-existing API integration test failures resolved (`#942` -- `Connectors:EncryptionKey` for Production-mode tests, MCP telemetry log entry assertion).
