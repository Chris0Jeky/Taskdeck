...
## Active delivery checkpoint (2026-07-27, durable desktop bootstrap configuration)

- **`#1242` product slice:** non-headless Production now resolves `appsettings.local.json` into the same durable OS app-data directory as the resolved SQLite database and threads that one path through web, HTTP-MCP, and stdio-MCP startup. An absent durable file imports the complete executable-local legacy JSON once through cross-process locks, owner-only staging, and an atomic non-overwriting move; an existing durable file always wins, while the retained legacy recovery source is also restricted. Corrupt recovery evidence, path/source races, permission failures, and ambiguous winners fail closed. Development, staging/test, and headless Production retain the executable-local path; the headless supplied-key contract is unchanged.
- **Data-loss guard and CLI parity:** API and CLI resolve the effective database file before generating secrets and refuse replacement connector/JWT material when an existing database has no recoverable supplied or persisted connector key. CLI-generated local config is co-located with its resolved SQLite file. Shared file-lock and owner-only file-security helpers keep the API/CLI persistence contracts aligned. Exact local evidence before publication: build clean at 0 errors (four pre-existing warnings), migration 34/34, API first-run 44/44, CLI bootstrap 26/26, Architecture 20/1 intentional skip, YAML/Bash/diff/DCO gates clean. Full serialized backend, independent final review, and exact-head hosted gates remain outstanding.
...
- **Windows secrets-at-rest ACL:** `#1263` (`#1241`) locks `appsettings.local.json` (JWT secret + connector encryption key) to the current user — `RestrictFileToCurrentUser` (Unix `0600` / Windows protected owner-only DACL, dependency-free on net8.0) runs create→restrict→write in `PersistValue`, fail-closed (an unrestrictable file is never written; callers fall back in-memory), with startup forward remediation for pre-`#1241` installs and a locked-down `.corrupt-*` backup. Review rounds also hardened the **MCP launch paths**: both MCP modes load the same absolute path as the web API (durable app-data for desktop Production after `#1242`, executable-local elsewhere — MCP clients may launch stdio servers from arbitrary CWDs; corrupt-file quarantine; env-vars keep priority over the file). Seeded: `#1262` (CLI bootstrapper sibling), `#1264` (atomic create-with-ACL — delivered as `#1267`).
