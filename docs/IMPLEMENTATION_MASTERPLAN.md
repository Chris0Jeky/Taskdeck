...
- **Release sequencing is unchanged.** No v0.1.2 tag exists. Exact-main no-publish run `32679854224` and an isolated synthetic Mock core-loop journey passed on an earlier main; the post-`55dbf6e14` final synthetic desktop receipt-to-explicit-Apply journey has not run. The open Priority I milestone tranche, maintainer deck acceptance, public-artifact verification, inherited-profile migration, and ordinary Explorer/SmartScreen acceptance remain ahead of a tag. ProjectV2 plus the milestone stay authoritative for the live queue.
