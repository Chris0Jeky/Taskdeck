...
## Delivery update (2026-08-21, truthful source launcher lifecycle)

- **`#1875` is delivered for the source-checkout path, not packaged-desktop release proof.** PR `#1891` enforces Node `>=24.13.1 <25`, unambiguous Windows npm resolution, and unconditional lockfile reconciliation. PR `#1892` proves the complete Vite entry graph and reports the actual resolved URL/port. PR `#1893` runs the serial launcher contract before frontend installation in Required CI on Ubuntu and Windows.
- **PR `#1896` preserves and ships the complete transactional lineage while superseding closed-unmerged PR `#1894`.** A fresh Development-only response identity binds readiness to the launched API. Run-bound `demo:seed` proves that identity and keeps every later mutation on the same non-reconnectable HTTP/1.1 socket, closing `#1895` without changing Production, packaged, MCP, generic-web, or manual no-run-ID behavior.
- **Exact-head proof is bounded and direct.** At `fdcbc34142ce3554f550dca6597dcb4808d2d502`, launcher tests passed 75/75, health tests 17/17, demo-seed tests 33/33, typecheck/static/syntax/DCO/diff checks passed, and the scoped production review finished CRITICAL 0 / HIGH 0. A real isolated Windows custom-port seed/restart/stop journey also passed with exact readiness identity, persisted login, state cleanup, and port release. Hosted checks completed 22 success / 11 skipped / 0 failed across Required CI run `32439078169` and secondary release-contract run `32439078282`.
- **Residuals remain separate.** `#1897` tracks explicit response deadlines for a peer that accepts but never completes a request; this is availability, not an identity bypass. `#1898` tracks the late review's bounded zombie/lock/environment/termination/test hardening without reopening the wrong-instance defect. `#1140` remains open for broader workspace cleanup. Explorer/shortcut/default-browser/SmartScreen, clean-Windows acceptance, and public-release authority remain `#1876`/owner gates.
