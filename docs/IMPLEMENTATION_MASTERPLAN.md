...
- **Transcript triage visibility (`#1574`):** the frontend keeps one two-second triage poll active for up to 15 minutes instead of stopping after 30 seconds, reducing stale `Triaging` views for default hosted-provider map-reduce work without changing terminal/error/cancellation behavior or the review-before-apply model. Provider-aware polling deadlines for slower supported configurations remain an explicit follow-up in `#1585`.
