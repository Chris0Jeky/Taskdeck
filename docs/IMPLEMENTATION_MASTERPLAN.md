...
- **The hosted gate found two real blockers and both were cleared the same day.** PR `#2110` needed one bounded fix round after both API matrices exposed the concurrent-delete 500; the repaired head `0af890a3` passed exact-head Required CI and E2E before merge `d75d0783`. `#2111`/PR `#2112` landed the Windows source-launcher harness repair (merge `36a055395`), root-causing the EBUSY teardown race to a live process holding the fixture root as its working directory plus an MSYS-pid kill unsynchronized with native exit, and replacing it with a wait-until-removable deadline teardown that names surviving native pids. PR `#2102` then merged on the repaired base (merge `709a098a7`). Five deferred launcher P2s are tracked on `#2136` and a Linux/macOS stub-server shutdown gap on `#2135`.
