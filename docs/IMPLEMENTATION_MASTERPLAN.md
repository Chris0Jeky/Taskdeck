...
- **Project priority completeness (`#1458`):** replace the capped ProjectV2 item sample with cursor-complete, stable-snapshot pagination and fail-closed checks for saturation, item/cursor identity, nested-connection truncation, project/source-plan drift, invalid issue priority labels, and ambiguous repository-aware PR references. The authentication-free self-test covers strict fallback, reference ordering, and zero-write apply guards; `-Apply` validates every option and rebuilds the complete derivation plan before the first write. Delivery remains data-gated: a complete 1,458-item read found 130 issues without a single priority label, and the canonical PR pass stops on referenced issue #123's missing Priority label before producing a trusted write plan. No `-Apply` ran; #1458 stays open until labels are classified and a post-apply audit proves zero drift.
