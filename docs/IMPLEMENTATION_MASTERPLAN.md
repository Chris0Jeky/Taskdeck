...
- **Merged PR `#1908` closes `#1903` by composing separate mandatory clean-install and migration journeys in the current gate.** The archive harness passed 26/26 and the release-structure contract passed 60/60; full parsed-config equality preserves every synthetic section/key/value, including the non-identity sentinel, while allowing formatting-only runtime reserialization. Exact-head no-publish run `32453648121` passed both journeys and the untouched-ZIP gate, then skipped release creation. It does not establish a public v0.1.1 release or manual clean-machine acceptance.
...
- **Exact-main automated recertification passed at `f9d9b84c`.** Required CI run `32488801335` accepted all 18 jobs, including dynamic E2E Smoke. No-publish run `32488879613` built and tested the untouched inner Windows ZIP, published artifact `9449039716`, and skipped release creation. The downloaded ZIP's SHA-256 is `d4e5ea79eb8932e0a434ef7630fa5e3eb88377564b16f72730def7dfc10b79af`; a separate required local `-LiveOpenAI` run passed schema-6 clean-install, migration, live triage, review-first apply, and restart persistence while the archive remained byte-identical. This is the automated release-candidate boundary, not a public release or clean-machine manual acceptance.
