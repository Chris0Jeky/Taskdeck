...
- **Two review ceilings remain durable parked lineages, not fix cascades.** `#1967`/PR `#2025` preserves the earlier broad applied-proposal detail lineage at `a70f88ad`; its identity, diff, and unavailable-state blockers were extracted into merged PRs `#2034`, `#2056`, `#2066`, and `#2070`. `#1973`/PR `#2031` remains preserved at `ecf44ea2` after its single fix round still left Paper capture history without a full detail surface and allowed a retained proposal hash to escape the archived read-only boundary. Resume from the issue comments and preserved branches, not by reopening or rewriting those PR heads.
- **Two next-session successors are clean, rebased, and remotely preserved without claiming shipment.** `#1949` directive-aware attribute tokenization is `origin/issue-1949/directive-attribute-tokenization` at `d89bd7cc3`; `#1967` applied read-only decision records are `origin/issue-1967/applied-read-only-detail` at `010021a745`. Each is one commit over main `55dbf6e14`, has no open PR, and still owes its published review/hosted gate. The applied-detail slice additionally owes a full frontend/Playwright or manual browser pass.
...
- **Cold-start continuation order:** (1) open/review the saved `#1949` tokenizer head and ship it if its refreshed exact-base gates pass; (2) independently review and publish the saved `#1967` applied-detail head, then run its browser seam; (3) run one final synthetic desktop capture -> review receipt -> explicit Apply -> exactly-one-board-card journey plus a blank-tag no-publish Windows candidate on the resulting exact main; (4) reconcile `#1938`, `#1940`, `#1947`, `#1949`, `#1967`, and the separately gated `#1242`, `#1973`, `#1992`, and `#2004` before preparing the maintainer release deck. Refresh Git, GitHub, ProjectV2, CI, review threads, and worktrees before acting; this list is a checkpoint, not a permanent status mirror.
