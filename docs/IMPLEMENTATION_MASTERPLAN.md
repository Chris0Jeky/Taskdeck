...
- **OpenAI-compatible LLM runtime (`#1306`, live-vendor acceptance pending):** add a distinct, opt-in `OpenAICompatible` selection path for public OpenAI Chat Completions-compatible endpoints, with true SSE deltas, explicit persisted degradation, conversational buffered fallback, and the bounded `response_format` compatibility retry. The hardened local head requires public HTTPS outside gated `http://localhost` development, disables ambient proxies only for this client, refuses all redirects, preserves URL plus connection-time DNS/IP SSRF defenses, and ensures egress-envelope violations and logs contain sanitized origins only. When authoritative usage is absent after upstream dispatch, quota settlement conservatively commits the original reservation estimate; buffered and streaming calls share success/failure and half-open probe state without treating ordinary degraded finishes as transport failures; LF/CRLF/CR SSE is accepted; and total response time, buffered/aggregate bytes, line bytes, and event bytes are bounded. The concrete transport is internal and production construction is confined to the reviewed API registration boundary. Compose, Render, and configuration/setup docs expose the same API key/base URL/model/deadline/byte-budget contract plus optional non-secret `HTTP-Referer` and `X-Title` attribution for OpenRouter, Groq, and DeepSeek. The 277-pass/1-existing-skip focused lane and full backend solution (7,570 pass/5 skip/0 fail) are green locally; exact-head hosted review/CI and a real-vendor visibly incremental stream still gate completion, so this does not close `#1306`.
