...
**4 PRs merged + 2 dependabot groups** — the three feature/substrate PRs below plus `#1447` (the docs sync recording the maintainer-merged `#1414` stored-preview presentation) — (per-PR gate: two independent adversarial review lenses, all-severity fixes with posted evidence, full backend suite on the exact head where code changed, CI green, bot window + content sweep):
...
- **Dependabot:** `#1443` (.NET 8.0.29 patch train) + `#1440` (npm minor-patch group) merged. Maintainer-owned leftovers: `#1441` (actions group, workflow-file policy) staged green; `#1442` (pinia 3→4 major) red — needs a real migration slice or an explicit close-to-snooze.
- **Process/estate notes:** duplicate tracker `#1450` consolidated into `#1444`; Gemini Code Assist (consumer) is sunset and Copilot review quota-exhausted — Codex is the only live external review bot. The frugality-overlay promotion into `docs/agentic/OVERNIGHT_LOOP.fable.md` is PR `#1448` (its merge state is tracked on the PR, not here).
