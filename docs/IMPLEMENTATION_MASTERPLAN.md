...
- **Current evidence boundary:** the exact documented filters pass 66/66 API and 151/151 Application tests; registered EventSource and scoped-Sentry controls pass three fresh-process repetitions; Architecture passes 20 with one intentional skip. Two independent code/security reviews were clean before the docs-only current-main merge. The merged Dockerless Testcontainers repair is absorbed. Docs/golden/diff, full serialized backend, exact-current-head review, signed publication, Required CI, CodeQL, and merge remain outstanding, so this is not yet a shipped delivery entry.
