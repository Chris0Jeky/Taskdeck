...
- **Six bounded product/deployment issue families are shipping incrementally.** `#1876`/PR `#2016` ships the secret-free ordinary-profile retired-provider diagnostic and explicit-migration compatibility rule; `#1966`/PR `#2017` makes board-scoped Inbox/Review state visible and clearable; `#1938`/PRs `#2023` + `#2033` preserve failed quick-capture drafts and add persistent expandable/copyable error receipts with an explicit Paper dismiss path; `#1949`/PRs `#2027` + `#2032` + `#2038` + `#2055` extend the dead-affordance source guard through labelled custom-button keyboard semantics and case-sensitive Vue modifiers; `#1967`/PRs `#2034` + `#2056` resolve canonical proposal identities and show an explicit non-actionable unavailable state for missing Paper deep links; and `#1992`/PRs `#2029` + `#2030` correct lowercase installed-PWA and split-container routing for the machine prefixes. The latter four parent issues remain open for their explicitly recorded residual acceptance criteria.
