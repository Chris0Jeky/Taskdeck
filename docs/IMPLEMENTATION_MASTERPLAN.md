...
- **Project priority completeness (`#1458`):** replace the capped ProjectV2 item sample with cursor-complete, stable-snapshot pagination and fail-closed checks for saturation, item/cursor identity, nested-connection truncation, project/source-plan drift, invalid issue priority labels, typed repository-aware PR references, partial writer failure, and stale post-apply reporting. The authentication-free self-test covers typed reference ordering, strict fallback, zero-write preflight, partial failure auditing, and verified post-state output; `-Apply` validates every option and rebuilds the complete derivation plan before the first write. Delivery remains data-gated: a complete 1,458-item read found 130 project issues without a single priority label. The typed PR pass ignores misidentified PR #123, then fails closed on actual external Issue `dotnet/efcore#35983`; the referring project item remains unidentified because the API rate limit expired. No trusted PR plan or `-Apply` exists; #1458 stays open pending truthful project labels, source identification, an external-reference policy decision, and a final zero-drift audit.
