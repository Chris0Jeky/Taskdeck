...
- **Capture board pre-filter pushed into SQL:** `#1254` (`#1239`) gives `GetCapturesByUserAsync` an optional `boardId` that applies `(BoardId IS NULL OR BoardId = @boardId)` in-query (SQLite raw + EF), so a board-filtered capture scan no longer materializes other boards' captures; null-board rows are retained because applied-conversion provenance (resolved in the service) can still map them to the target board, and the effective-board + status filters stay in the service. A behavior-preserving optimization (the SQL pre-filter is an exact superset of the removed in-service raw-board check).
