...
- **Evidence bounds to carry forward, not to round up.** Each slice passed its focused suites and hosted Required CI at its exact head - `#2073` at 21/21 after one E2E rerun cleared a chat-surface timing flake unrelated to its seam - **except `#2077`, whose Windows `Frontend Unit` lane concluded `cancelled` rather than completed, so `E2E Smoke` skipped**, while its Ubuntu lane and every other Required lane passed. No slice in this wave was browser- or Playwright-verified: the archived-history, membership, chat-honesty, and applied-record behaviors all rest on unit, integration, and component evidence. Several consecutive post-merge `main` runs were cancelled by the next merge landing, so the final synthetic desktop capture -> review -> explicit Apply journey and a blank-tag no-publish Windows candidate are still owed on the resulting exact main before any release deck.
...
- **Both saved successor heads have since been published and merged.** *(Corrected later on 2026-08-24.)* `#1949` directive-aware attribute tokenization (`origin/issue-1949/directive-attribute-tokenization`, `d89bd7cc3`) became PR `#2072`; `#1967` applied read-only decision records (`origin/issue-1967/applied-read-only-detail`, `010021a745`) became PR `#2073`. Neither is a saved-but-unpublished head any longer, and neither still owes a published review or hosted gate. The applied-detail slice still owes a full frontend/Playwright or manual browser pass.
