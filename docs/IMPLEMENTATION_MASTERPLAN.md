...
- platform, ops, testing, and maturity backlog: `#84` to `#111`, `#87` to `#91`; PWA/offline readiness delivered (`#95`): `vite-plugin-pwa` + Workbox `generateSW`, a precached app shell, NetworkFirst API caching, CacheFirst same-origin static assets, StaleWhileRevalidate lazy locale chunks, SPA navigation fallback, `useOnlineStatus`, and the `OfflineBanner` / `SwUpdatePrompt` AppShell surfaces. The generated manifest retains its installability fields, while packaged delivery still needs the `manifest-src` CSP repair tracked by `#2045`; offline behavior is documented in `docs/platform/PWA_OFFLINE_BEHAVIOR.md`; 18 original readiness tests (11 composable + 7 component)
