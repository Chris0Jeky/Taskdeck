...
- **Still open on the tracker.** `#1939` (Today seal state machine and honest empty states) is PR `#1976`, open and unmerged. `#1936` stays open for its revised AC2/AC4 product decisions; `#1951` (Paper pre-triage capture edit) and `#1965` (add-column affordance — fix on `main`, issue not auto-closed) remain open; `#1640`'s server-vs-UI same-day question is untouched by this wave.
