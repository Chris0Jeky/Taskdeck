...
22 visual tests (expanded from 7 in TST-59, `#865`/`#948`): archive; auth (login/register); board components (toolbar/action rail); board modals (card, column, starter pack); board view (empty + populated); calendar; capture modal; command palette (open + search); home; inbox; metrics; notifications; review; settings; shell sidebar; and today. Clock pinning via `page.clock.install()` for date-dependent views; `document.fonts.ready` wait for font-load determinism; dynamic content hidden via `data-testid="timestamp"`, session identity/presence selectors, the session-timeout alert, and the `[data-toast-id]` toast stack, all through the `hideDynamicContent` helper. **Baselines must be captured on the hosted Linux lane** — Chromium font rasterisation differs enough between platforms that a locally captured set fails on every image; the bootstrap and review procedure is in the policy. Policy at `docs/testing/VISUAL_REGRESSION_POLICY.md`.
