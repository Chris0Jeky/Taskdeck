...
- `docs-governance`
  ...
  - Windows `Worktree Helper (Windows PowerShell)` runs
    `powershell -NoLogo -NoProfile -NonInteractive -File scripts/git/Test-New-CodexIssueWorktree.ps1`
    as a 26-case harness enforcing detached-first creation, clean source and exact-blob-pinned
    handoff artifacts, fail-closed initialization, and permission-contract regressions
