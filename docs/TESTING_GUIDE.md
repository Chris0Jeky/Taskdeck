...
selection, explicit checkout-repository pinning and `GH_REPO` rejection, immutable selected-OID
ancestry, enforcing-workflow provenance, exact opening-base scan-definition equality, and
...
opening-state snapshot, bootstrap-safe `readlink` handling, raw-byte cleanliness that cannot be
forged by repository-local clean filters, the redirected token-file channel, and fail-fast proving
commands. Transient replacement-ref and
branch-ref races against definition binding run against a real Git checkout. The unlinked-runtime
rejection checks the security property without assuming a platform-specific diagnostic. Twelve
cases first assert that a deliberately defective variant fails them (thirteen defective variants
in all), so the canary cannot pass by accident. Ensure `jq` and `rg` resolve outside the checkout;
native Git Bash supplies the remaining POSIX tools, including `perl`:
