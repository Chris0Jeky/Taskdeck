...
- **The OpenAI, Gemini, Ollama, and outbound-webhook primary clients now preserve the validated direct origin without exporting protected request details through Taskdeck's configured telemetry.** Each `SocketsHttpHandler` sets `UseProxy = false`, so ambient/system proxy settings cannot replace the `OutboundWebhookConnectCallback` target. The provider selected for a development localhost opt-in reuses the same startup-resolved policy as selection and connect-time validation. These four clients remove default `IHttpClientFactory` request loggers, disable distributed-trace header propagation, and use a private HTTP metric scope; Taskdeck's OpenTelemetry configuration drops marked activities and that scope, including `server.address`/`server.port`. This is not a guarantee against independently installed process-global activity or meter listeners. Existing strict localhost opt-ins and automatic-redirect blocking are unchanged. Proxy-aware outbound support does not exist, so corporate proxy-only deployments fail closed; this slice does not broaden redirect or audit behavior.
- **The review-fix working tree is local and not yet shipped:** 63/63 affected API tests pass across registered pipelines, fully decoded provider dispatch bodies, Production policy injection, proxy canaries, delivery behavior, and telemetry; 151/151 provider and selection-policy compatibility tests pass. The 6-case provider-dispatch lane passed five consecutive repetitions and the 2-test registered telemetry lane passed three. Docs governance, golden-principles governance, GitHub-operations governance, and `git diff --check` also pass. The prior 7,511-pass full-suite result predates this repair and is not exact-head evidence; full serialized backend verification, final signed publication, exact-head re-review, Required CI, and CodeQL remain outstanding.
