...
- Implemented automation stack:
  ...
  - MCP server: current packages pin `ModelContextProtocol` 1.4.1 with the full inventory (`#653`/`#739`); 9 resources under `taskdeck://`; 11 tools — five board-mutating tools create proposals, `create_capture` writes directly only to Inbox, and `dismiss_proposal` directly closes an owned proposal; no approve/apply tool is exposed. Stdio uses `StdioUserContextProvider`. **MCP HTTP transport** (`#654`/`#792`, boundary repair `#1338`): standalone and co-hosted modes share only `/mcp`; `ApiKeyMiddleware` validates `tdsk_` Bearer keys, `HttpUserContextProvider` maps the key to its user for resource/tool authorization, telemetry wraps the real route, and the 60 req/60s policy partitions by opaque key ID. Standalone defaults to loopback with loopback-only host filtering; `/` is not MCP.
