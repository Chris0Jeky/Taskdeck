...
- **The live project is not currently priority-clean.** A read-only snapshot at project `updatedAt=2026-07-26T00:25:24Z` collected all 1,458 items over 15 pages and then correctly stopped: 130 issue items had no priority label (83 `Done`, 5 `Review`, 42 `Pending`) and an empty project Priority. A preliminary pass identified six PR fallback candidates; the next pass falsely treated merged PR #123 as an unlabeled issue, which re-review caught. The typed rerun ignores that PullRequest and then reaches external Issue `dotnet/efcore#35983`, which GitHub reports as a closing reference for 12 historical Taskdeck Dependabot PRs (#494–#1113) because their release-note text links it. Cross-repository Issues now fail closed by default so an external label taxonomy cannot become Taskdeck authority without a reviewed opt-in. The preliminary PR plan remains retracted pending truthful project-issue labels and any external-reference policy decision. No audit remediation writes were applied, so `#1458` remains open.
