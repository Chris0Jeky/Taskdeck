...
Stored-preview review presentation (2026-07-18, **1 PR maintainer-merged** after the frugal wave — `#1414`, closes `#1397`; frontend-only, per the maintainer decision recorded on `#1397`): expired and terminal proposals stay inspectable but no longer fire a live diff that `400`s — both review shells (Legacy `useReviewActions`/`ReviewProposalCard` and Paper `PaperReviewView.onPreviewDiff`) short-circuit read-only proposals to the stored `diffPreview` under an explicit read-only banner (or a "no stored preview available" note), the expired-path button is relabeled "View/Hide stored preview", and a `400` from `/diff` renders an explicit **invalid** state (pending zero-op proposals show "contains no operations; Apply would reject it" instead of an approvable "No changes" surface). Server-authoritative expiry is honored and stored-preview reveals are re-authorized on the `#1414` P2 round. New shared helpers: `isProposalReadOnly` (`useReviewProposals`) + `isValidationError` (`useErrorMapper`). The backend `400` contract from `#1395` is unchanged. Residual review P2s are tracked as `#1436` (merged-with-tracked per the maintainer's call).
