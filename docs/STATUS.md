...
- **The Inbox keeps its existing two-second triage poll active for a bounded 15-minute window rather than stopping after 30 seconds.** That keeps default hosted-provider map-reduce work visible while retaining terminal/error/cancellation handling, one active poll, manual refresh, and the proposal-first review boundary. Provider-aware polling deadlines for slower supported configurations are deliberately tracked separately in `#1585`.

Transcript schema-v2 contract delivery (2026-08-02, REVIVAL-08 M3 contract slice / `#1304`):
- **LLM transcript triage now uses a strict, server-stamped `llm-triage.v2` contract while the deterministic fallback remains v1.** Model output must contain an exact task property set: title, lowercase type (`action`/`decision`/`question`), nullable bounded assignee and calendar-date hints, bounded confidence, and a bounded nonblank evidence quote. Unknown, duplicate, missing, wrongly typed, noncanonical, or over-limit output falls back for the whole capture/map leg rather than being normalized, truncated, or partially retained. Each quote must be an ordinal substring of the exact provider chunk; the existing proposed-card description preserves it for review. Classification, assignee, due-date, and confidence metadata remain transient and are deliberately excluded from executable proposal parameters. This slice adds no migration, API/UI evidence surface, identity resolution, or automatic board mutation; `#1305` remains open for durable linkage, spans, provenance reads, and Paper deep links.

Post-merge reconciliation (2026-08-01, PRs `#1556`, `#1558`, and `#1559`):
- **REVIVAL-09 now has an independent durable transcript persistence foundation, but `#1305` remains open.** A user-owned `Transcript` stores normalized LF text and optional line-indexed speaker/timestamp segments. It may reference a board, originating capture, and `SourceArtefact`; deleting the board or artefact nulls the corresponding FK without deleting the transcript. The current transcript-capture path also retains input text in `LlmRequest.Payload`; retention and portability audits must treat both stores as durable until `#1305` links triage to `Transcript` and establishes the final ownership boundary. User-scoped deterministic Transcript reads, buffered/streaming data-portability export, account deletion, a reversible EF migration, and fresh-database bootstrap coverage shipped in PR `#1556` (`bfd4e95a`, merge `3182c3ac`). Remaining `#1305` scope is triage persistence/linkage, evidence spans, additive provenance API work, and Paper deep-link UI. Current-main focused verification passed 5 Domain, 63 Application, and 14 API/migration tests.
...
<!-- 2026-07-17: REVIVAL-02 (#1298, Phase 1 truth + safety) — removed the fake Paper review undo timeline: the review UI no longer advertises an undo window/countdown ("Undo window" / "window closes"), an undo keyboard shortcut, or undo-rate copy, because no revert endpoint exists anywhere in the backend. (The Today cover's "Auto-seals in…" copy is the separate, real day-sealing feature — deliberately untouched.) The stable side-effect `reversibility` wire shape (summary/description/windowMs) is preserved for GP-03 contract compatibility, but the frontend now maps it to factual apply-risk guidance ("Apply considerations") and the confidence label renders "Operation safety" while keeping the `/confidence` transport key unchanged. `PaperUndoTimeline` component + spec deleted; executor/apply behavior untouched; truthful undo-adjacent copy (destructive-action cannot-be-undone warnings, reversible archive restore, transaction rollback) deliberately preserved. -->

Security runway slice (2026-07-26, `#1345`):
- **The inherited HIGH SQLite native advisory is remediated without bypassing Central Package Management.** `Taskdeck.Infrastructure` now directly references the centrally pinned `SQLitePCLRaw.bundle_e_sqlite3` 2.1.12, so bundle, core, provider, and native library resolve as one 2.1.12 family instead of mixing a new native leaf with 2.1.6 managed components. The loaded SQLite runtime is 3.53.3, above the advisory's 3.50.2 security floor; a host-native integration test opens an in-memory database and enforces that floor. The solution vulnerability audit is clean, a fresh database update reports no pending model changes, and a self-contained `win-x64` published API booted against SQLite and returned `/health/ready` HTTP 200.
...
Canonical UI + historical archive period (2026-06-13; direction superseded 2026-07-10):
- **Historical direction only:** from 2026-06-13 until ADR-0044 on 2026-07-10, the project direction was finish-for-personal-use, then archive, with no distribution. **That posture no longer controls execution.** The revival direction at the top of this file and `docs/REVIVAL_PLAN.md` are authoritative; previously de-scoped distribution work is re-triaged only where the ratified REVIVAL wave admits it.
...
- **Review deep-dive backends** (6 endpoints on `AutomationProposalsController`, required by PAPER-06 `#1002`):
  ...
  - Side-effect analysis (`#1020`/`#1033`): `GET /api/automation/proposals/{id}/side-effects` returns 7-category breakdown (Cards/Subtasks/Comments/Activity/Notifications/Webhooks/Calendar) with active/passive tone and risk-based reversibility window (6h default, 3h for Critical); review fixed target-type checks and webhook-without-operations logic; 66 tests *[2026-07-17: the `windowMs` field is legacy review-attention metadata, not an undo capability — no revert endpoint exists; copy/semantics reframed to apply-risk guidance by REVIVAL-02 `#1298`, wire shape unchanged]*
...
> **⚠️ HISTORICAL — superseded first by the 2026-06-13 archive pivot and then by ADR-0044.** This 2026-02-18 backlog split (Phase-4 completion + future-expansion tranches, `#33`-`#111`) is **not** the active issue-selection order. Do **not** pull work from these tranches as "current backlog" — the live order is the ratified REVIVAL wave in `docs/REVIVAL_PLAN.md`. Retained only as a record of the pre-pivot roadmap shape.
...
- rehearsal contract is now delivered (`#355`); GTM baseline (demo script, landing copy, beta intake workflow) was delivered (`#216`). ADR-0044 makes all three relevant planning assets again, subject to a current ship-gate review before public use.
...
- `#216` for broader demo script / public framing (delivered: `DEMO_SCRIPT.md`, `LANDING_COPY.md`, `BETA_INTAKE_WORKFLOW.md`; ADR-0044 reactivates them as beta-planning assets, with a fresh ship-gate review required before public use)
...
- Default policy is review-first for all risk levels; `AutoApplyLowRisk` may bypass tool-use review only for low-risk, proposal-producing tools and never bypasses ADR-0003's explicit proposal approve-then-execute gate for a board write
...
- originally-planned concepts that were tracked in roadmap docs (**several since delivered — see annotations**):
  ...
  - explicit release framing for `R1` novice-first beta, `R2` agent foundation alpha, and `R3` knowledge/integrations alpha _(historical — this old R1/R2/R3 ladder was de-scoped during the 2026-06-13 archive period and is not revived; ADR-0044's bounded v0.1–v0.3 beta horizon lives in `docs/REVIVAL_PLAN.md`)_
