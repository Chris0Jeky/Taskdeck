...
- **Still open — T4 human gate, but not ready for canaries:** `#1457` remains draft with red DCO on all four commits, no substantive review of its current head, and a branch materially behind `main`. At the 2026-07-26 audit it pinned dispatcher v1.6.5 while both the installed floor and agent-harness producer were v1.6.12; Codex's common-directory/root hook-source topology also remained unresolved. Repair and rebase/merge the branch, settle the exact reviewed pin/source, then rerun exact-head CI and two reviews before asking the maintainer for a fresh `/hooks` trust session and allow/deny canaries.
