...
- **Paper is the canonical UI** per **ADR-0038** (records the #1136 decision; the default-theme flip is not yet shipped). **Activation prerequisites now shipped (2026-06-19):** the `#1161` "File away" dismiss affordance (`#1219`), the shared review-actionability composable (`#1217`), light-theme straggler tokenization (`#1216`), and a **real in-app Appearance theme toggle** (`#1221`, `AppearanceSettingsView` at `/workspace/settings/appearance` — off / paper / paper-night / auto) which replaces the unlinked `/styleguide/paper` route as the reachable Legacy escape hatch. **Remaining before the default-theme flip:** the Paper-review de-stubs (`onPreviewDiff` / `onDefer` / `onReportBadSuggestion`). **All new frontend UX work targets the Paper views; Legacy is frozen** (no new fixes). The ~26 routes without Paper variants keep rendering Legacy views inside the Paper shell after a contrast pass.
