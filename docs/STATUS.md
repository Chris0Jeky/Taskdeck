...
Durable desktop bootstrap configuration (2026-07-27, `#1242`):
- **A packaged desktop upgrade no longer loses the connector-encryption key or JWT secret when the executable moves.** Non-headless Production resolves one `appsettings.local.json` under the same OS app-data directory as its resolved SQLite database; web, HTTP-MCP, and stdio-MCP startup all reuse that exact path. When the durable file is absent, first run locks both paths and atomically imports the complete executable-local legacy JSON without overwriting an existing winner; the legacy recovery copy is retained and both secret-bearing files must be owner-only. Corrupt-recovery evidence, permission failures, source changes, and ambiguous concurrent outcomes fail closed. Development, staging/test, and headless Production keep the historical executable-local path, and headless Production still requires an operator-supplied connector key.
- **Existing databases cannot be paired silently with replacement secrets.** Before generating either secret, API and CLI resolve the effective SQLite file path and stop if that database exists but no supplied or persisted connector key is recoverable. The CLI stores its generated local config beside the resolved database. Environment and other higher-priority sources still override the generated file without overwriting its recoverable key. Exact local evidence before publication: solution build succeeded; local-config migration 34/34, first-run bootstrap 44/44, CLI bootstrap 26/26, and Architecture 20 passed / 1 intentional skip; full serialized backend and hosted gates remain the publication gate.

Dockerless Testcontainers gate repair (2026-07-27, `#1518`):
- **The PostgreSQL integration collection now reaches its documented graceful-skip path before Testcontainers validates Docker.** `PostgresContainerFixture` checks `DockerAvailableCheck` before constructing the container, so a Dockerless host runs seven host-native integration tests and reports all 28 container-dependent tests skipped instead of throwing from the fixture constructor. The real public-constructor and injected-factory regressions prove that the unavailable path does not construct or dispose a phantom container. A timed-out Docker CLI probe is killed with its process tree and given a bounded reap before the check returns; the local exact tree finished with 7 passed / 28 skipped / 0 failed and zero `docker info` processes before or after. Exact-head hosted Container Integration remains the positive startup/connection/teardown proof and passed 30 / 0 / 0 before the final review-fix commit, so it must rerun on the new head.
