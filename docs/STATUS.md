...
Overnight wave (2026-07-23/24, **4 PRs merged** + 2 dependabot groups — the three feature/substrate PRs below plus `#1447`, the docs sync that recorded the maintainer-merged `#1414` (its content is the 2026-07-18 post-merge comment further down); per-PR gate: two independent adversarial review lenses, all-severity findings fixed with posted evidence, full backend suite on the exact head where code changed, required CI green, bot window honored with a content sweep):
...
- **Dependabot:** `#1443` (.NET 8.0.28 → 8.0.29 patch train + Test.Sdk 18.8.1) and `#1440` (npm minor-patch group incl. vue 3.5.40) merged green. **Left for the maintainer:** `#1441` (actions group — workflow files are maintainer-merge-only) staged green; `#1442` (pinia 3→4 MAJOR) is genuinely red and needs a scheduled migration slice, not a rubber stamp.
...
- **Seeded this wave:** `#1429` (memory-capped extraction worker process — the ADR-0048 containment boundary), `#1431`, `#1433`, `#1434`, `#1435`, `#1436`. **Still open — NOT shipped:** `#1427` (LLM quota-reservation robustness hardening, `#1313`) is in the final review gate, its atomicity redesign tracked as `#1435`; standing human-gated holds `#1295` (deny-floor overlay) and `#1337` (licensing pack).
