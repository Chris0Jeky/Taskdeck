...
- **REVIVAL-09 now has a durable transcript persistence foundation, but `#1305` remains open.** A user-owned `Transcript` is the sole durable home for normalized LF text and optional line-indexed speaker/timestamp segments. It may reference a board, originating capture, and `SourceArtefact`; deleting the board or artefact nulls the corresponding FK without deleting the transcript. User-scoped deterministic repository reads, buffered/streaming data-portability export, account deletion, a reversible EF migration, and fresh-database bootstrap coverage shipped in PR `#1556` (`bfd4e95a`, merge `3182c3ac`). Remaining `#1305` scope is triage persistence/linkage, evidence spans, additive provenance API work, and Paper deep-link UI. Current-main focused verification passed 5 Domain, 63 Application, and 14 API/migration tests.
