...
- **Configured Python-backed agent handlers no longer depend on the unusable Microsoft Store `python` aliases on native Windows.** Configured PowerShell handlers and Windows allow-rules use the verified `py -3 -B` launcher; POSIX allow-rules and guidance retain `python3 -B`; and the smoke harness runs child hook scripts through its active `sys.executable -B`. The smoke contract asserts the exact five-utility by two-launcher permission matrix. No Python installation, global PATH change, or product-runtime packaging is introduced.
