...
- **The candidate makes both manual Compose phases self-contained without creating a deployment boundary.** Each job generates a 48-byte JWT secret and 32-byte connector encryption key with the runner's cryptographic RNG, masks both before writing them to its job-scoped environment file, and fails closed unless the Compose file's required-variable inventory is exactly those two names. The former `production` environment job is replaced by a summary-only parked handoff that binds no GitHub environment and creates no deployment.
- **Evidence and boundary:** the combined Node/governance suite passes 19/19; the real Compose file renders with generated inputs and fails when the connector key is withheld; docs, Golden Principles, and GitHub-operations governance pass. Local Actionlint, hosted exact-head CI, and a real `workflow_dispatch` rehearsal have not yet run. The child branch remains dependent on parked PR `#1502`; environment protection, activation, and merge remain maintainer-owned under `#1504`.
