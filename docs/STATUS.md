...
- **In review (not yet merged):** `#1219` (`#1161`) — Paper review "File away" dismiss affordance built on the `#1217` shared-actionability foundation (per-proposal filing rail in terminal states + bulk "File away N settled" + dual-purpose ⌫); adversarial-reviewed, all findings fixed, awaiting CI/aging.
...
- multi-tenancy strategy ADR is documented (`#71`) with shared-schema + `TenantId`; the cross-user **isolation** behaviour (`TenantId` on entities + the `403`/`404` existence policy) is live and remains the security model, while further **multi-organization tenant slices are de-scoped** by the 2026-06-13 archive pivot (single-user, local-first — see the Direction note above and ADR-0004)
