...
- **The PostgreSQL integration collection now reaches its documented graceful-skip path before Testcontainers validates Docker.** `PostgresContainerFixture` checks `DockerAvailableCheck` before constructing the container, so a Dockerless host runs the two host-native integration tests and reports all 28 container-dependent tests skipped instead of throwing from the fixture constructor. A deterministic factory-seam regression proves the unavailable path neither constructs nor disposes a phantom container; the Docker-backed CI lane remains the positive startup/connection/teardown proof.
