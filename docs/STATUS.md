...
- **The current-main review-fix working tree is local and not yet shipped:** the documented exact filters pass 66/66 API and 151/151 Application tests across registered pipelines, provider compatibility, proxy canaries, delivery, EventSource, OpenTelemetry, and scoped-Sentry boundaries. The registered EventSource and Sentry positive/negative controls passed three fresh-process repetitions; Architecture passes 20 with one intentional skip. Two independent code/security reviews were clean before the docs-only current-main merge. Docs governance, golden-principles governance, full serialized backend verification, exact-current-head review, signed publication, hosted CI, and CodeQL remain outstanding.
