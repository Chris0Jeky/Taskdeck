...
- **security bug discovered 2026-04-03**: `#722` (SEC-20) — `ChangePassword` endpoint does not verify that the caller's JWT identity matches the supplied `UserId`; any authenticated user can change another user's password. **No external user onboarding should proceed until this is fixed.**
...
- Implemented automation stack:
  ...
  - `BoardMetricsService` (throughput, cycle time, WIP, blocked — audit-log-based completion tracking, done column name heuristic) + `MetricsController` with date/board/label filters
  - MCP server baseline: `ModelContextProtocol` v1.2.0, `BoardResources` with `taskdeck://boards` resource, `--mcp` startup flag for stdio transport, `StdioUserContextProvider` for local user mapping
