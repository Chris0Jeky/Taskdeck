...
- ~~**security bug discovered 2026-04-03**: `#722` (SEC-20) Ã¢â‚¬â€ `ChangePassword` endpoint does not verify caller identity~~ **RESOLVED** (`#722`/`#732`, 2026-04-04): `ChangePassword` now derives userId exclusively from JWT claims; `[Authorize]` enforced; `UserId` removed from request body; `AuthController` inherits `AuthenticatedControllerBase`; 5 integration tests proving the fix
...
- rigorous test expansion wave seeded 2026-04-03 (`#721` tracker, 22 issues `#699`Ã¢â‚¬â€œ`#726`): systematic codebase audit identified 25+ untested infrastructure repositories, zero tests on the central worker, 6 controllers with untested HTTP surfaces, and no golden-path integration test for the capture Ã¢â€ â€™ proposal Ã¢â€ â€™ board pipeline; execution is tracked in `docs/TESTING_GUIDE.md`; first delivery: infrastructure repository integration tests (`#699`/`#730` Ã¢â‚¬â€ 77 tests across 7 repo classes against real SQLite); **major wave delivery 2026-04-04** (PRs `#732`Ã¢â‚¬â€œ`#739`, 8 issues, ~300 new tests): SEC-20 ChangePassword fix (`#722`/`#732`), golden-path captureÃ¢â€ â€™board integration test (`#703`/`#735` Ã¢â‚¬â€ 7 tests proving full pipeline), cross-user data isolation tests (`#704`/`#733` Ã¢â‚¬â€ 38 tests across all major API boundaries), LlmQueueToProposalWorker integration tests (`#700`/`#734` Ã¢â‚¬â€ 24 tests, previously zero coverage), controller HTTP integration tests (`#702`/`#738` Ã¢â‚¬â€ 67 tests covering 6 untested controllers, found 2 pre-existing bugs), proposal lifecycle edge cases (`#708`/`#736` Ã¢â‚¬â€ 74 tests for state machine/expiry/race conditions), OAuth/auth edge cases (`#707`/`#737` Ã¢â‚¬â€ 44 tests, found and fixed `Substring` overflow bug in `ExternalLoginAsync`), MCP full resource/tool inventory (`#653`/`#739` Ã¢â‚¬â€ 9 resources + 11 tools with 42 tests, GP-06 compliant, user-scoping gap fixed during review); **second wave delivery 2026-04-04** (PRs `#740`Ã¢â‚¬â€œ`#755`, 8 issues, ~586 new tests with two rounds of adversarial review, 47 review-fix commits): domain entity state machine exhaustive tests (`#701`/`#740` Ã¢â‚¬â€ 174 tests across 7 entities: CommandRun, ArchiveItem, ChatSession, UserPreference, NotificationPreference, CardLabel, CardCommentMention), SignalR hub and realtime integration tests (`#706`/`#751` Ã¢â‚¬â€ 19 tests covering auth, presence lifecycle, multi-user, authorization, edge cases), LLM provider abstraction and tool-calling edge cases (`#709`/`#747` Ã¢â‚¬â€ 101 tests across orchestrator, provider, classifier, registry), data export/import round-trip integrity tests (`#713`/`#752` Ã¢â‚¬â€ 64 tests covering JSON, CSV, GDPR, database, cross-format validation), API error contract regression and boundary validation (`#714`/`#753` Ã¢â‚¬â€ 57 tests across 7 endpoint families with GP-03 contract enforcement), archive and restore lifecycle integration tests (`#715`/`#755` Ã¢â‚¬â€ 74 tests: 45 domain + 29 API covering state machine, cross-user isolation, conflict detection, audit trail), board metrics and analytics accuracy verification (`#718`/`#749` Ã¢â‚¬â€ 61 tests: 51 service + 10 controller covering throughput, cycle time, WIP, blocked cards, done-column heuristic), notification delivery, deduplication, and preference filtering (`#719`/`#746` Ã¢â‚¬â€ 36 tests covering all 5 notification types, deduplication, preference filtering, cross-user isolation, batch operations)
...
- UX feedback wave 3 delivered (2026-04-01): review proposal card UX improved (`#613`/`#665`) Ã¢â‚¬â€ sticky action footer with `position: sticky; bottom: 0` keeps action buttons visible regardless of card content length, cards constrained to `max-height: 70vh` (80vh mobile) with internal scrolling, detail section heights capped at `12rem`
...
- Hardening and UX wave delivered (2026-04-03, PRs `#691`Ã¢â‚¬â€œ`#698`): 9 issues resolved across 8 PRs with adversarial review follow-through:
  - **P1 bug fixed** (`#681`/`#691`): Archive, Activity, Ops, and Access workspace routes no longer silently redirect to Home Ã¢â‚¬â€ feature flags for shipped surfaces now default to `true`; 5 new router guard tests
...
- Post-hardening delivery wave (2026-04-03, PRs `#724`Ã¢â‚¬â€œ`#731`): 6 issues resolved across 6 PRs:
...
- Security + testing + MCP delivery wave (2026-04-04, PRs `#732`Ã¢â‚¬â€œ`#739`): 8 issues resolved across 8 PRs with two rounds of adversarial review:
  ...
  - **Golden-path integration test** (`#703`/`#735`): 7 tests exercising full capture Ã¢â€ â€™ triage Ã¢â€ â€™ proposal Ã¢â€ â€™ review Ã¢â€ â€™ board pipeline; validates card title, column placement, provenance chain, multi-operation atomicity, cross-user isolation, audit trail, and triage failure determinism
...
- Post-adversarial-review hardening and test expansion wave (2026-04-04, PRs `#741`Ã¢â‚¬â€œ`#756`, 9 issues):
...
- Tech-debt, security, and feature hardening wave (2026-04-04, PRs `#765`Ã¢â‚¬â€œ`#770`, `#776`, 7 issues, ~32 new backend tests + 33 new frontend tests, two rounds of adversarial review per PR):
...
- Dependency hygiene, accessibility, tool-calling refinements, streaming, and test coverage wave (2026-04-04, PRs `#771`Ã¢â‚¬â€œ`#779`, 8 issues, ~258 new tests with two rounds of adversarial review per PR):
...
- Feature, analytics, MCP, chat, testing, and UX expansion wave (2026-04-08, PRs `#787`Ã¢â‚¬â€œ`#793`, 7 issues, ~390+ new tests with two rounds of adversarial review per PR):
...
- Post-validation hardening and expansion wave (2026-04-16, PRs `#822`, `#841`, `#844`, `#877`–`#880`, `#882`):
  - **E2E scenario expansion round 2 fixes** (`#712`/`#822`): Playwright keyboard navigation, dark mode stubs, and onboarding/review/capture edge-case fixes from round 2 review; 20 scenarios across 5 spec files refined for CI reliability
  - **INT-06 merge conflict resolution and normalization tests** (`#340`/`#841`): resolved merge conflicts in integrations registry foundation; added connector direction/status/type enum normalization test coverage for frontend integration layer
  ...
  - **MCP API key management UI** (`#655`/`#877`): frontend settings page at `/workspace/settings/api-keys` for managing MCP API keys (scope item 5 of `#655`); `apiKeysApi.ts` API client module with typed list/create/revoke functions; active/revoked key listing, create dialog with one-time key display + clipboard copy, revoke confirmation dialog; uses Td* design system primitives and `--td-*` design tokens; 19 tests (6 API client + 13 view component)
  - **Cloud deployment preparation** (`#538`/`#878`): production Dockerfile (`deploy/Dockerfile.production`) with multi-stage .NET 8 + Vue 3 SPA build, non-root user, HEALTHCHECK, runtime PORT env var for managed platforms; Railway config (`deploy/railway.toml`), Render Blueprint (`deploy/render.yaml`), env template (`deploy/.env.production.template`); deployment guide at `docs/platform/CLOUD_DEPLOYMENT_GUIDE.md`; single-container architecture (API serves SPA from `wwwroot/`); single-instance SQLite constraint documented with PostgreSQL migration path referenced
  - **MCP structured logging and observability** (`#655`/`#879`): `McpOperationLogger` for structured `ILogger` output on MCP tool calls and resource access; `McpTelemetryMiddleware` with OpenTelemetry `ActivitySource` spans, request/error counters, and duration histograms; `LogSanitizer` for sensitive data protection (API keys and request bodies never logged); integrates with existing `TaskdeckTelemetry` ActivitySource and tag conventions; 4 new test files covering logging behavior and data protection
  - **INT-04 connector framework foundation** (`#98`/`#880`): `IConnectorProvider` abstraction with `ConnectorCapabilities` and `ConnectorHealthResult` in Domain; `ConnectorProviderRegistry` and `ConnectorExecutionService` in Application layer; `ConnectorCredential` entity with AES-256-GCM encryption at rest via `AesCredentialEncryptionService`; EF Core migration for `ConnectorCredentials` table; `GitHubConnectorProvider` pilot (context type, read-only repo/issue/PR info); `ConnectorProvidersController` with provider discovery and credential management endpoints; inbound connectors route through capture per GP-06; 26 new tests across Domain, Application, and credential encryption
  - **CI test failure root cause fixes** (`#882`): resolved 5 pre-existing CI test failures — CORS tests (3) and HSTS test (1) failing because Production environment validates JWT secret is not a placeholder (fixed by supplying non-placeholder `Jwt:SecretKey` via `UseSetting`); board presence concurrency test timeout increased from 10s to 20s with all-snapshot scanning for expected member count; full backend suite (4,477 tests) passes with 0 failures
...
- Extended controllers: auth, users, board-access, audit, export/import, external-imports, llm-queue, automation proposals, archive, chat, notifications, ops-cli, logs, health, starter-packs, search, metrics, data-portability, note-import, telemetry, api-keys, forecast, mfa, oidc, integrations, connector-providers
- Worker runtime:
...
- Implemented automation stack:
  ...
  - `AbuseDetectionService` with `AbuseActor`/`AbuseEvent` domain entities and a 4-state containment model (Observe Ã¢â€ â€™ Suspicious Ã¢â€ â€™ Restricted Ã¢â€ â€™ Blocked); operator kill-switch API groundwork for SEC-18
  ...
  - `DataExportService` (versioned JSON export of all user-scoped data; streaming export via new `GET /api/account/export/stream` endpoint using `Utf8JsonWriter` for memory-constant large-dataset exports Ã¢â‚¬â€ `#670`/`#774`; exception logging via `ILogger` with `OperationCanceledException` filter, `#759`/`#766`) + `AccountDeletionService` (password re-auth, confirmation phrase, PII anonymization, sole-owner guard, transactional rollback with `CancellationToken.None` for rollback reliability) + `DataPortabilityController` with audit logging
  ...
  - Connector framework (`#98`/`#880`): `IConnectorProvider` abstraction with `ConnectorCapabilities` and `ConnectorHealthResult` in Domain; `ConnectorProviderRegistry` for provider discovery and `ConnectorExecutionService` for execution orchestration in Application layer; `ConnectorCredential` entity with AES-256-GCM encryption at rest via `AesCredentialEncryptionService`; `GitHubConnectorProvider` pilot (context type, read-only repo/issue/PR info); `ConnectorProvidersController` with provider discovery and credential management endpoints; inbound connectors route through capture per GP-06
  ...
  - SignalR realtime baseline: `BoardsHub` with board-scoped subscription authz and application-level board mutation event publishing; **scale-out readiness** (`#105`/ADR-0023): conditional Redis backplane via `Microsoft.AspNetCore.SignalR.StackExchangeRedis` 8.0.25 Ã¢â‚¬â€ enabled when `SignalR:Redis:ConnectionString` is configured, falls back to in-memory when absent; `RedisBackplaneHealthCheck` reports NotConfigured/Healthy/Unhealthy in `/health/ready`; operational runbook at `docs/platform/SIGNALR_SCALEOUT_RUNBOOK.md`
  - OpenTelemetry baseline for API + worker metrics/traces with configurable OTLP/console exporters; MCP structured logging (`McpOperationLogger`, `McpTelemetryMiddleware`, `LogSanitizer`) adds MCP-specific spans, counters, and histograms to the observability pipeline (`#655`/`#879`)
...
- Workspace routes include:
  ...
  - settings (profile/preferences/access/export-import/linked-accounts/mfa-setup/telemetry-consent/api-keys)
  - archive
...
- Large view decompositions (hotspot refactor wave):
  - `ActivityView.vue` decomposed from ~735 Ã¢â€ â€™ ~117 lines via `useActivityQuery` composable + `ActivitySelector` + `ActivityResults` components
  - `BoardView.vue` decomposed from ~771 Ã¢â€ â€™ ~270 lines via `useBoardDragDrop` + `useBoardKeyboardNav` composables + `BoardToolbar` + `BoardActionRail` + `BoardCanvas` + `BoardDialogHost` components
...
## Platform Expansion Wave (2026-04-09, PRs `#796`Ã¢â‚¬â€œ`#805`, 10 issues)
...
## Feature, Security, and Ops Expansion Wave (2026-04-09, PRs `#806`Ã¢â‚¬â€œ`#813`, 8 issues)

Eight parallel worktree agents delivered new features, security infrastructure, ops tooling, and developer experience improvements across 8 PRs. Each PR received two rounds of adversarial review (original self-review + independent cold review). The independent round caught 9 CRITICAL and 11 HIGH findings Ã¢â‚¬â€ all resolved before merge.
...
Estimated totals after PRs `#821`–`#826` supplementary wave + PRs `#837`–`#841` validation/integrations wave: backend ~4,530+, frontend ~2,463+, E2E 61+ new scenarios, combined ~7,070+ passing. After PRs `#877`–`#882` post-validation wave: backend 4,477+ (recertified via `#882` full-suite pass) + ~49 new tests (MCP logging 4, connector framework 26, API key UI 19), frontend ~2,482+, combined ~7,120+ estimated.
...
- demo rehearsal runtime issues (2026-03-27): seed idempotency blocker (`#387`), scenario `--skip-llm` blocker (`#389`), DX friction (`#388`, `#390`), narrative mismatch (`#394`), and polish (`#391`, `#392`, `#393`) Ã¢â‚¬â€ tracked in `#395`
...
- `#368` chat live-provider status and first-turn fidelity Ã¢â‚¬â€ degraded message type, probe health, verified UI state
- `#369` headed manual-audit Playwright pack (`Priority IV` by design)

Reused existing anchor:
- `#326` proposal readability Ã¢â‚¬â€ affected entity labels now show named targets instead of raw IDs, correlation IDs truncated in UI
...
Dependency updates (PRs `#593`Ã¢â‚¬â€œ`#600`):
- `@eslint/js` 9.39.4 Ã¢â€ â€™ 10.0.1 (with ESLint v10 rule violation fixes in demo scripts and playwright config)
- `@types/node` 24.10.1 Ã¢â€ â€™ 25.5.0
- GitHub Actions group bump (5 updates)
- `Microsoft.NET.Test.Sdk` 17.14.1 Ã¢â€ â€™ 18.3.0
- `Swashbuckle.AspNetCore` 6.9.0 Ã¢â€ â€™ 10.1.7 (with OpenApi v2.x compatibility fix); exported OpenAPI artifact needs regeneration (`#609`)
- `Microsoft.IdentityModel.Tokens` and `System.IdentityModel.Tokens.Jwt` upgraded to 8.17.0
- `xunit.runner.visualstudio` 2.8.2 Ã¢â€ â€™ 3.1.5
...
- `#238` SEC-18 abuse detection + automated containment (`Priority III`) -- **operator tooling + domain groundwork delivered**: `AbuseActor`/`AbuseEvent` entities, `AbuseDetectionService` (4-state ObserveÃ¢â€ â€™SuspiciousÃ¢â€ â€™RestrictedÃ¢â€ â€™Blocked model), operator evaluation/quarantine/unquarantine/block API; live-traffic automated containment wiring is a follow-up slice
...
Verification Date: 2026-03-31 (recertified after PRs #588Ã¢â‚¬â€œ#607 merge wave)
...
Result:
- Frontend unit: 1491/1491 passing (134 test files) Ã¢â‚¬â€ **stale**: post-wave count is 1592/1592 (~125 files); see `docs/TESTING_GUIDE.md` for latest estimates
- Typecheck: passing
...
- cloud deployment preparation is now shipped (`#538`/`#878`): production Dockerfile (`deploy/Dockerfile.production`) with multi-stage .NET 8 + Vue 3 SPA single-container build; Railway config (`deploy/railway.toml`) and Render Blueprint (`deploy/render.yaml`); env template with JWT placeholder; deployment guide at `docs/platform/CLOUD_DEPLOYMENT_GUIDE.md`; single-instance SQLite constraint documented with PostgreSQL migration path
...
- Delivered UI-01 follow-up (PR `#455`): DRY accent-color refactor in `design-tokens.css` Ã¢â‚¬â€ 9 hardcoded hex values replaced with `--_td-light-accent` and `--_td-light-accent-hover` CSS variables; single source of truth for the primary action accent.
...
- Delivered DOC-05 / SEC-17 managed-key usage policy (PR `#458`): `docs/security/MANAGED_KEY_USAGE_POLICY.md` Ã¢â‚¬â€ user-facing fair-use limits, prohibited patterns (scraping, bulk operations, key extraction), enforcement ladder (warn Ã¢â€ â€™ restrict Ã¢â€ â€™ suspend Ã¢â€ â€™ ban), and appeals process; linked from active security docs.
...
- Delivered BoardView decomposition (PR `#463`): `BoardView.vue` reduced from ~771 Ã¢â€ â€™ ~270 lines via `useBoardDragDrop` (column/card DnD logic), `useBoardKeyboardNav` (j/k/h/l keyboard selection), `BoardToolbar.vue` (header, presence, filter, settings actions), `BoardActionRail.vue` (board-context action strip), `BoardCanvas.vue` (column DnD scaffold + ColumnLane), `BoardDialogHost.vue` (all modal/overlay hosting); unit + component tests added.
- Delivered UI-02 shared UI primitives foundation (PR `#464`): 15 shared primitive components in `src/components/ui/` Ã¢â‚¬â€ `TdButton`, `TdIconButton`, `TdInput`, `TdTextarea`, `TdSelect`, `TdFieldWrapper`, `TdDialog`, `TdDropdown`, `TdPopover`, `TdTooltip`, `TdToast`, `TdInlineAlert`, `TdSkeleton`, `TdSpinner`, `TdBadge`, `TdTag`, `TdEmptyState`; built on Reka UI via shadcn-vue copy-paste ownership; WAI-ARIA keyboard foundation throughout.
- Delivered OUT-01 JSON manifest import tab (PR `#465`): `StarterPackCatalogModal.vue` gains a JSON Import tab with paste/file-upload, validateÃ¢â€ â€™dry-runÃ¢â€ â€™apply flow; JSON payload parsed against the v1 manifest schema with actionable error display before apply.
...
- Delivered SEC-18 abuse detection operator tooling + domain groundwork (PR `#468`): `AbuseActor.cs` + `AbuseEvent.cs` domain entities with 4-state containment model (Observe Ã¢â€ â€™ Suspicious Ã¢â€ â€™ Restricted Ã¢â€ â€™ Blocked); `AbuseDetectionService` with signal evaluation, state-machine transitions, operator quarantine/unquarantine/block/list API groundwork; live-traffic wiring is an explicit follow-up slice.
