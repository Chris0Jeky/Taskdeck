...
Archive-pivot delivery wave (2026-06-13, **17 PRs merged** across Waves 0–2, including 2 follow-on dependabot bumps `#1211`/`#1212`; full review gate per PR — 2 independent adversarial reviews, all-severity findings fixed, Gemini/Codex/Copilot bot threads resolved, fresh green CI + aging):
...
- **Seeded follow-ups:** `#1209` (no sweeper recovers non-capture `LlmRequests` stuck in Processing), `#1215` (`FieldVerifier`/`DeterministicPreExtractor` registered-but-unconsumed after V1 removal — keep-for-V2 decision), `#1218` (repo-wide focus-ring contrast sweep).
- **Reviewed + green + aged; merges held by maintainer (2026-06-19):** two Paper-activation PRs are fully through the gate (2 independent adversarial reviews, all-severity findings fixed, fresh green CI, aged) and are **not yet merged only because the maintainer is holding merges**, not because work remains:
  - `#1219` (`#1161`) — Paper review "File away" dismiss affordance built on the `#1217` shared-actionability foundation (per-proposal filing rail in terminal states + bulk "File away N settled" + dual-purpose ⌫). `#1161` is checked off in `OUTSTANDING_TASKS.md` (2026-06-19).
  - `#1221` — reachable in-app **Appearance** theme toggle (off / paper / paper-night / auto), the Legacy escape hatch and the real in-app switch that the Paper-canonical activation prerequisites require (replacing the unlinked `/styleguide/paper` route).
  - (`#1220` itself — this docs-reconciliation PR — is the third held PR.)
...
- originally-planned concepts that were tracked in roadmap docs (**several since delivered — see annotations**):
  ...
  - ~~`Agents`, `Runs`, `Knowledge`, and `Integrations` product surfaces~~ — **delivered** (`AgentsView`/`AgentRunsView`/`AgentRunDetailView` AGT-03 `#338`; `KnowledgeDocument` + `KnowledgeFtsSearchService` `#339`; `IntegrationsView` + `IntegrationRegistryService` INT-06 `#340`); this "planned" listing is historical
