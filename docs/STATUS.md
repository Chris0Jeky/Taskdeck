...
- **Taskdeck no longer installs repo-owned Claude or Codex runtime hooks.** `.claude/settings.json` retains normal development permissions, worktree symlinks, and project MCP enablement, but has no `hooks` object or `permissions.deny` list; the root has no `.codex/hooks.json`. The T3 authority declaration remains in `.agent-harness/tier.json`, and Required CI, global laws, worktree guards, plus any independently configured user/organization/runtime controls remain effective. Absence of project hooks is not a claim that those external layers are disabled.
