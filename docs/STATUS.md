...
- Docs, auth, tech-debt, testing, and CI hardening wave (2026-04-23, PRs `#936`--`#942`, 7 issues):
  - **Docs cleanup** (`#934`/`#936`): duplicate `#326` entry in `ISSUE_EXECUTION_GUIDE.md` annotated as intentional follow-up pass; legacy `TASKDECK_E2E_DB` env var prefix removed from E2E commands across 5 active docs files (CLAUDE.md, README.md, CONTRIBUTING.md, MANUAL_VERIFICATION_CHECKLIST.md, MANUAL_TEST_CHECKLIST.md) to align with TESTING_GUIDE.md
  - **UTF-8 mojibake fix** (`#929`/`#937`): 109 double-encoded UTF-8 sequences in `STATUS.md` replaced with correct Unicode characters (46 em dashes, 17 en dashes, 46 right arrows)
  - **Redundant migration no-op** (`#932`/`#938`): `20260422183834_AddExternalLoginsUserForeignKey` migration `Up`/`Down` bodies replaced with empty no-ops; FK was already added in `20260402230822_AddTokenInvalidatedAt`; Designer.cs and model snapshot untouched
  - **Auth refresh endpoint** (`#933`/`#939`): `POST /api/auth/refresh` endpoint added to `AuthController` with `[Authorize]`, per-user rate limiting (5 req/60s via `TokenRefreshPerUser` policy), defense-in-depth active/existence checks; completes the session extension feature started in `#919` (FE-20); 10 integration tests; `docs/platform/CONFIGURATION_REFERENCE.md` updated with rate limit policy
  - **Test count recertification** (`#930`/`#940`): TESTING_GUIDE.md updated with verified counts from full suite runs on `main` at `97d4856c` -- backend 4,979 passing (was ~4,760+ estimated), frontend 2,607 passing (was ~2,482+ estimated), combined 7,586+; growth since last recertification (2026-04-12): backend +700, frontend +362
  - **UserPreferences race condition fix** (`#931`/`#941`): `UserPreferenceRepository.GetOrCreateDefaultByUserIdAsync` replaced try-catch-retry pattern (105 lines) with SQLite `INSERT OR IGNORE` upsert (51 lines); eliminates `UNIQUE constraint failed` log noise in E2E CI; 4 new integration tests including 10-task concurrent gate test
  - **Pre-existing CI test failures resolved** (`#942`): 5 API Integration tests failing on all PRs fixed -- 4 CORS/HSTS tests lacked `Connectors:EncryptionKey` when switching to Production mode (appsettings.Development.json not loaded), 1 MCP telemetry test asserted against wrong log entry (started vs completed); `ApiTestHarness.TestEncryptionKey` constant added for Production-mode test setup
  - FE-20 session timeout warning is now fully functional end-to-end: frontend `useSessionTimeout` composable calls `POST /auth/refresh`, backend issues a fresh JWT with extended expiry
...
- Cross-cutting UI infrastructure:
  ...
  - session timeout warning: `useSessionTimeout` composable with configurable warning-before-expiry, countdown timer, and extend/logout actions; `SessionTimeoutWarning.vue` wired into `App.vue`; backend `POST /auth/refresh` delivered in `#933`/`#939` — session extension now fully functional end-to-end
