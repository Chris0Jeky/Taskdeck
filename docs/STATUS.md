...
Overnight wave (2026-07-23/24, **4 PRs merged** + 2 dependabot groups — per-PR gate: two independent adversarial review lenses, all-severity findings fixed with posted evidence, full backend suite on the exact head where code changed, required CI green, bot window honored with a content sweep):
