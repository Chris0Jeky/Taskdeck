...
- Hardened local frontend manual startup (`npm run dev`) with deterministic port fallback (`5173` -> `4173` -> `5001`), identity-verified listener reuse, and strict-port startup so restricted `5173` environments no longer fail or drift through implicit Vite port auto-increment.
- Resolved frontend container-image `npm ci` policy blocker by pinning `ws` via npm overrides to `8.19.0`, removing the forbidden `ws-7.5.10` tarball request from the lock-resolved dependency graph.
