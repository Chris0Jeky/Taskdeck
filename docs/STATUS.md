...
- **`#1395` (`#1376`) — proposal diff preview now equals apply on every path.** `AutomationProposalService.GetProposalDiffAsync` runs the same gates apply runs (structure, then expiry) on all diff paths including the cached-`DiffPreview` fast path — expired → `400` "Proposal has expired", zero-op → `400` "Proposal must contain at least one operation" (was `200`/`404`). Extends the `#1374` preview==apply parity contract. Seeded `#1397` (frontend expired/zero-op preview UX + coverage) and `#1398` (fold `ValidatePermissionsAsync` access/existence gates into diff — the third asymmetry).
