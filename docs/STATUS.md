...
- **The OpenAI, Gemini, Ollama, and outbound-webhook primary clients now preserve the validated direct origin without exporting protected request details through Taskdeck's configured telemetry.** Each `SocketsHttpHandler` sets `UseProxy = false`, so ambient/system proxy settings cannot replace the `OutboundWebhookConnectCallback` target. The provider selected for a development localhost opt-in reuses the same startup-resolved policy as selection and connect-time validation. These four clients remove default `IHttpClientFactory` request loggers, disable distributed-trace header propagation, and use a private HTTP metric scope; Taskdeck's OpenTelemetry configuration drops marked activities and that scope, including `server.address`/`server.port`. When Sentry is enabled, Taskdeck disables its automatic `IHttpClientFactory` handler so it cannot reintroduce trace/baggage propagation, URL breadcrumbs, or failed-request capture on those protected clients; server-side Sentry exception tracking remains enabled. This is not a guarantee against independently installed process-global activity or meter listeners. Existing strict localhost opt-ins and automatic-redirect blocking are unchanged. Proxy-aware outbound support does not exist, so corporate proxy-only deployments fail closed; this slice does not broaden redirect or audit behavior.
- **The review-fix working tree is local and not yet shipped:** 73/73 affected API tests pass across registered pipelines, fully decoded provider dispatch bodies, Production policy injection, proxy canaries, delivery behavior, and OpenTelemetry/Sentry boundaries; 151/151 provider and selection-policy compatibility tests pass. The 6-case provider-dispatch lane passed five consecutive repetitions; the enabled-Sentry four-client success/5xx regression passed ten consecutive repetitions after its disabling control failed as expected. The regression now closes its process-global Sentry hub and runs nonparallel. Docs governance, golden-principles governance, GitHub-operations governance, and `git diff --check` also pass. The Dockerless Testcontainers substrate repair `#1518` is merged and absorbed into this branch; full serialized verification on this new branch head, final security-boundary review, signed publication, exact-head hosted CI, and CodeQL remain outstanding.

Dockerless Testcontainers gate repair (2026-07-27, `#1518`):
- **The PostgreSQL integration collection now reaches its documented graceful-skip path before Testcontainers validates Docker.** `PostgresContainerFixture` checks `DockerAvailableCheck` before constructing the container, so a Dockerless host runs seven host-native integration tests and reports all 28 container-dependent tests skipped instead of throwing from the fixture constructor. The real public-constructor and injected-factory regressions prove that the unavailable path does not construct or dispose a phantom container. A timed-out Docker CLI probe is killed with its process tree and given a bounded reap before the check returns. PR `#1519` merged as `ff1ee066` after local Integration passed 7 / 28 / 0 with zero `docker info` processes before and after, the serialized backend solution passed 7,465 / 33 / 0, exact-head hosted Container Integration passed 35 / 0 / 0, all hosted gates passed, and two independent reviews plus Codex were clean. Future fail-closed zero-skip enforcement remains tracked as `#1520`.
...
- **Configured Python-backed agent handlers no longer depend on the unusable Microsoft Store `python` aliases on native Windows.** Configured PowerShell handlers and PowerShell-scoped utility allow-rules use the verified `py -3 -B` launcher; POSIX Bash allow-rules and guidance retain `python3 -B`; and the smoke harness runs child hook scripts through its active `sys.executable -B`. The permission contract asserts five direct utilities plus the renderer synchronization unittest on each platform's actual tool surface. PreToolUse maps a missing launcher or failed policy process to fail-closed exit 2. The native-Windows smoke feeds `Bash` payloads through PowerShell-hosted handlers; it does not prove native PowerShell-tool interception, which remains the T4 gap tracked by [#1497](https://github.com/Chris0Jeky/Taskdeck/issues/1497). No Python installation, global PATH change, or product-runtime packaging is introduced.
