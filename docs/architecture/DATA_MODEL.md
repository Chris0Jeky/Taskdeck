...
The durable, user-owned Inbox object (ADR-0065 §Decision 1, amended 2026-08-30). Scaffolded
2026-08-30 and reconciled the same day: the table exists and is written only while
`ContextFabric:DualWriteCaptures` is on (default off), as an ID-preserving mirror of the legacy
`LlmRequest` capture row; Inbox reads stay on the queue row until CF-01 `#2255` completes the
backfill and flips the read path. Job, run, representation, evidence-anchor and candidate tables
(CF-03, CF-06, CF-07, CF-08) hang off this row and are still not persisted.
...
(ADR-0065 §Decision 1, amended 2026-08-30). A capture holds **up to 32** assets
(`Capture.SourceAssets`, added through `AddInlineTextSource` / `AddSourceAsset`), so a sentence plus
a screenshot is one capture with two assets and routing runs per asset. Typed and pasted text is an
inline asset like any other: stored verbatim, never normalised, never trimmed onto a processing job
and never confused with the capture's user note. Nothing on an asset is mutated after intake.
...
**Indexes:** `(CaptureId, Ordinal)` (unique), `ContentHash`, `LegacyArtefactId`.
