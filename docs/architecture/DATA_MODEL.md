...
## Context Fabric (ADR-0065)

### Capture

The durable, user-owned Inbox object (ADR-0065 §Decision 1, amended 2026-08-30). Live since CF-01
`#2255`: `ContextFabric:DualWriteCaptures` defaults **on**, every capture is admitted through
`CaptureIntakeService` under the queue row's own id (ID-preserving), and every pre-existing
capture-shaped `LlmRequest` row is brought in by the startup backfill
(`CaptureBackfillService`, marker in `CaptureBackfillStates`). Inbox list and get now resolve a
capture's own material — its immutable source text, its `LegacySourceSnapshot` and its
`CapturedAtServer` — through `ICaptureStore` rather than parsing the queue payload; job state
(status, processed-at, retries, errors) and the fields with no column yet (triage provenance,
suggestion metadata, the disposition receipt) still come from the queue row, which is what keeps the
Inbox DTOs byte-identical across the switch. The switch is guarded on the backfill marker and
degrades per item, so a capture without a durable row is still read from its queue row. Job, run,
representation, evidence-anchor and candidate tables (CF-03, CF-06, CF-07, CF-08) hang off this row
and are still not persisted.

Capture state is **three orthogonal axes**, not one lifecycle column. `Disposition`
(`CaptureUserDisposition`) is the only axis a user sets, and `Archived` is terminal.
`ProcessingSummary` (`CaptureProcessingSummary`) and `ActionState` (`CaptureActionState`) are
projection columns: the first is rewritten from job records once CF-03 lands, the second from
planning records. The user-legible timeline (`CaptureTimelineStep`: Received, Preparing, Understood,
NeedsInput, NeedsReview, Acted, Kept, Failed, Archived) is **computed** by
`CaptureTimeline.Project(disposition, processing, action)` and exposed as the non-mapped
`Capture.Timeline`; it is never persisted, so no writer can drift the story from the state.

Identity and intent are likewise split. `UserId` is always the owner; `ProducedByPrincipalId` names
the producing principal only when that is somebody other than the owner, and `ProducerKind` is
`Human`, `Agent` or `Integration` — importing is a transport (`CaptureOriginAdapter.Import`), not a
principal, so the retired `Import` producer value is gone and `CaptureSourceMapping` maps
`Import`/`MarkdownImport` sources to a `Human` producer over the `Import` origin. `RequestedIntent`
is what the user asked for and may be `Auto`; `EffectiveIntent` is what a run resolved (never
`Auto`, null until then) with `IntentResolvedByRunId` recording which run decided.
`LegacySourceSnapshot` is a compatibility snapshot taken at intake and persisted — native captures
derive it once through `CaptureSourceMapping.ToLegacySource`, and it is not recomputed afterwards.
`PrimaryModality` summarises the first source asset so lists have one icon to draw; routing operates
per source asset, not on this field. Legacy `CaptureDisposition` values map through
`CaptureUserDispositionMapping.FromLegacy` (Kept → Kept, Archived → Archived, ProposalRequested →
Active), and to requested intent as ProposalRequested → Act, Kept → Remember, otherwise Organize.
...
| Field | Type | Required | Constraints | Description |
|-------|------|----------|-------------|-------------|
| Id | `Guid` | Yes | PK, never generated by the database | Equal to the legacy `LlmRequest.Id` for mirrored/backfilled rows |
| UserId | `Guid` | Yes | FK to User (Restrict) | Owning user |
| ProducedByPrincipalId | `Guid?` | No | | The producing principal when it is not the owner; null when the owner produced it |
| ProducerKind | `CaptureProducerKind` | Yes | Human, Agent, Integration | Server-stamped principal kind (`Import` retired — it is a transport) |
| CapturedAtServer | `DateTimeOffset` | Yes | | Server clock at intake (authoritative) |
| CapturedAtClient | `DateTimeOffset?` | No | | Client-reported time, a hint only |
| PrimaryModality | `CaptureModality` | Yes | Text, Audio, Image, Document, Structured | Summary of the first source asset, for lists; routing is per asset |
| OriginAdapter | `CaptureOriginAdapter` | Yes | WebComposer, FileUpload, ShareTarget, BrowserExtension, VsCodeExtension, Mcp, Import, Integration, Api | Transport/adapter provenance |
| RequestedIntent | `CaptureIntentMode` | Yes | Remember, Organize, Act, Auto | What the user asked for |
| EffectiveIntent | `CaptureIntentMode?` | No | Remember, Organize, Act — never Auto | What a run resolved an `Auto` request to; null until then |
| IntentResolvedByRunId | `Guid?` | No | | The run that resolved `EffectiveIntent`; no FK until CF-03 persists runs |
| Disposition | `CaptureUserDisposition` | Yes | Active, Kept, Archived; `Archived` is terminal | The only state axis a user sets |
| ProcessingSummary | `CaptureProcessingSummary` | Yes | Idle, Processing, Partial, Ready, Failed | Projection over job records (rewritten once CF-03 lands) |
| ActionState | `CaptureActionState` | Yes | Unplanned, NeedsInput, NeedsReview, Acted | Projection over planning records |
| LegacySourceSnapshot | `CaptureSource` | Yes | Defined enum value | Compatibility snapshot taken at intake (`CaptureSourceMapping`) |
...
| LegacyRequestId | `Guid?` | No | Unique | The queue row this capture was admitted or backfilled from; null for captures created natively once the queue row is retired (CF-03) |
| CreatedAt / UpdatedAt | `DateTimeOffset` | Yes | | |

`Timeline` is a non-mapped projection (EF ignores it), the same way `Transcript.Segments` is a
non-mapped JSON view. `SourceAssets` is the aggregate's owned collection of immutable inputs
(cascade delete, auto-included).

**Indexes:** `(UserId, CreatedAt)`, `(UserId, Disposition)`, `ContextBoardId`, `LegacyRequestId` (unique).

### SourceAsset

One immutable input of a `Capture` — the general source model between `Capture` and `Representation`
(ADR-0065 §Decision 1, amended 2026-08-30). A capture holds **up to 32 active** assets
(`Capture.SourceAssets`, added through `AddInlineTextSource` / `AddExternalReferenceSource` /
`AddSourceAsset`), so a sentence plus a screenshot is one capture with two assets and routing runs
per asset. Typed and pasted text is an inline asset like any other: stored verbatim, never
normalised, never trimmed onto a processing job and never confused with the capture's user note.
Nothing on an asset is mutated after intake.

**Corrections supersede, they never rewrite** (CF-01 `#2255`). A post-intake edit
(`Capture.SupersedeInlineTextSource`, reached from `CaptureService.UpdateSuggestionAsync`) appends a
new asset carrying `SupersedesAssetId` and stamps `SupersededByAssetId` on the one it replaces. The
original bytes stay exactly as the user gave them, `Capture.ActiveSourceAssets` and
`Capture.CurrentText` read the assets nothing has superseded, and the cap counts active assets only,
so a capture the shipped contract lets a user edit can never start failing because its correction
history grew.

`SourceArtefact` / `ArtefactBlob` are **adapted behind this model, not replaced**: an asset whose
bytes already live in the shipped artefact tables uses `StorageKind = LegacyArtefact` and a soft
`LegacyArtefactId` reference, so the artefact keeps its own retention until CF-23 moves it behind
the blob store. Factories: `FromInlineText` (defaults to `text/plain`, hashes the UTF-8 bytes),
`FromExternalReference` (`text/uri-list`), `FromBlobReference`, `FromLegacyArtefact`.

| Field | Type | Required | Constraints | Description |
|-------|------|----------|-------------|-------------|
| Id | `Guid` | Yes | PK, never generated by the database | |
| CaptureId | `Guid` | Yes | FK to Capture (Cascade) | Owning capture |
| Ordinal | `int` | Yes | Unique with `CaptureId`; from zero | Position within the capture; asset 0 decides `PrimaryModality` |
| Modality | `CaptureModality` | Yes | Text, Audio, Image, Document, Structured | What this asset carries |
| MediaType | `string` | Yes | 1-100 chars | Declared media type; it belongs to the asset, never to the stored bytes |
| ContentHash | `string` | Yes | 64 hexadecimal chars | Lower-case SHA-256 over the asset bytes (UTF-8 for inline text) |
| ByteSize | `long` | Yes | > 0 | Content size |
| StorageKind | `SourceAssetStorageKind` | Yes | InlineText, Blob, ExternalReference, LegacyArtefact | Where the bytes actually live |
| BlobReferenceId | `Guid?` | No | Set only for `Blob` | `IBlobStore` reference holding the bytes (CF-23) |
| LegacyArtefactId | `Guid?` | No | Set only for `LegacyArtefact`; references SourceArtefact (no FK) | Soft reference; the artefact keeps its own retention (ADR-0046) |
| ExternalReference | `string?` | No | Max 2048 chars | A locator the user supplied; never dereferenced at intake |
| OriginalName | `string?` | No | Max 255 chars | The name the input arrived with; provenance only |
| SupersedesAssetId | `Guid?` | No | Set on a correction; references SourceAsset (no FK) | The asset this one replaces |
| SupersededByAssetId | `Guid?` | No | Set on the replaced asset | The correction that replaced this one; null while this asset is current |
| CreatedAt | `DateTimeOffset` | Yes | | |
| UpdatedAt | `DateTimeOffset` | Yes | | |

**Indexes:** `(CaptureId, Ordinal)` (unique), `ContentHash`, `LegacyArtefactId`, `SupersedesAssetId`.

### SourceAssetTextPayload

Cold text payload for an `InlineText` asset, in a shared-primary-key one-to-one table so asset-list
reads never materialize up to 200,000 characters per asset — the same split `SourceArtefact` /
`ArtefactBlob` use for binary content.

| Field | Type | Required | Constraints | Description |
|-------|------|----------|-------------|-------------|
| SourceAssetId | `Guid` | Yes | PK and FK to SourceAsset (Cascade) | Owning asset |
| Text | `string` | Yes | 1-200,000 chars (`Transcript.MaxTextLength`) | The text exactly as the user gave it |

`SourceAssetTextPayload` inherits from `Entity` in the domain, but EF ignores the inherited `Id`,
`CreatedAt`, and `UpdatedAt`; `SourceAssetId` is the persisted identity.

### CaptureBackfillState

The marker that records whether the ID-preserving capture backfill has finished on this database
...
| Field | Type | Required | Constraints | Description |
|-------|------|----------|-------------|-------------|
| Id | `Guid` | Yes | PK, never generated by the database | Fixed singleton id per backfill key |
| Key | `string` | Yes | Max 100 chars, unique | Stable name of the migration tracked (`capture.legacy-queue.v1`) |
...
`IsComplete` is a non-mapped projection over `CompletedAt`.
...
Immutable metadata for a user-owned source. Binary content is stored separately so metadata reads do not materialize the blob. The 2026-08-30 Context Fabric reconciliation does not replace this table: an artefact is now reachable as the `LegacyArtefact` storage kind of a `SourceAsset` through the soft `SourceAsset.LegacyArtefactId` reference, and keeps its own shape, retention and FKs.
