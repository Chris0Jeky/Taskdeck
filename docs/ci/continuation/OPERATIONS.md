...
1. Resolve the conservative-contract edge cases tracked on #2336 before connecting admission: attempt gaps, unknown risk, explicit test inventory requirements, protected execution-inventory completeness, binding current producer workflow/test requirements to reused evidence, late baseline exposure, merge-delivery deduplication and same-second recovery ordering. Unknown transitive inputs keep a task reuse-ineligible. The [known admission gaps](ADMISSION_AND_LEDGER.md#auxiliary-decision-not-a-second-required-gate) must be fixed before authoritative reuse.
