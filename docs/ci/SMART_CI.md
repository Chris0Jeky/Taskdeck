...
**Merge-ref binding when the base moves.** `refs/pull/N/merge` is regenerated by GitHub against
whatever the PR's named base branch points at *now*. The resolver requires the event head
(`CONTROL_HEAD`) to match the merge ref's second parent **exactly**, and independently fetches the
exact named `--base-ref` tip before classifying the first parent. A merge ref is qualified only when
that first parent equals the authenticated named-base tip. `CONTROL_BASE` remains the trusted
control-code checkout identity; a stacked or non-default PR base need not descend from it.

GitHub may retain an exact-head merge ref after the named base advances. The resolver retries three
times to allow regeneration. If the old first parent still differs but GitHub's read-only compare API
positively proves it is in the current named-base history, the resolver emits only
`stale-base-unqualified`. It publishes no merge, tree, observed-parent, or base-tip qualification.
The resulting plan selects the full hosted estate; shadow mode stays green with `wouldFail: true`,
enforce mode is red, and recall explicitly excludes the observation as `merge-ref-unqualified`.
An unrelated parent, unreadable named base, failed or malformed ancestry proof, or any head mismatch
still fails closed with no outputs and becomes a `planner-error` plan. Notes remain explanatory text
and are never parsed as evidence.
...
The version-1 executable reader accepts legacy plans without `mergeRefQualification`. When both
`mergeBaseSha` and `mergeBaseTipSha` are also absent it uses `baseSha` as the historical parent
fallback and explicitly makes no claim that first-parent evidence was captured. A partial pair,
malformed SHA, or binding mismatch is invalid. New qualified PR plans carry
`mergeRefQualification: qualified` and all merge identities. A `stale-base-unqualified` plan carries
all four merge qualification identities as `null`; non-PR and error plans record the qualification as
`null`. Because the documentation schemas use `additionalProperties: false`, consumers must use the
updated schema before reading the new field.
...
first parent must be `baseSha` or the matching recorded `mergeBaseTipSha`. A typed unqualified stale
merge ref remains visible in the report but is unusable evidence. The landed merge commit must match
the final base, head and tree. Missing, duplicate, expired, stale or mismatched evidence fails closed.
A lane family is ready only after at least 20 usable merged PRs, an actual failure in that
