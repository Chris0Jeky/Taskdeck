...
k6 exits non-zero on threshold breach, failing the CI step. The tagged board-write profile measures SQLite's sustained 20-VU capacity at about 2000 ms p95; its 2200 ms hard gate is that measured capacity plus an explicit 10% CI jitter allowance, not an aspirational target. The `scripts/ci/check-k6-thresholds.mjs` script parses the k6 JSON summary and emits a near-capacity `::warning` at or above 2000 ms while retaining the aggregate, read, error-rate, and check-rate hard gates.
