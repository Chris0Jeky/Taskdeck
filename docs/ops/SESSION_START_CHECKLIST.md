...
2. Run required checks:
   ...
   - PowerShell frontend sequence: `Push-Location frontend/taskdeck-web; npm run typecheck; if ($LASTEXITCODE -ne 0) { Pop-Location; exit $LASTEXITCODE }; npm run build; if ($LASTEXITCODE -ne 0) { Pop-Location; exit $LASTEXITCODE }; npx vitest --run --maxWorkers=2; $code = $LASTEXITCODE; Pop-Location; if ($code -ne 0) { exit $code }`
