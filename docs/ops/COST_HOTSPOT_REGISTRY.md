...
| Scaling behavior | **Superlinear** Ã¢â‚¬â€ each chat message may trigger 1-5 tool-calling rounds, each round is a full API call with growing context window. A single complex conversation can cost 5-10x a simple one. Capture triage adds per-item LLM cost. |
...
| Risk level | **High** Ã¢â‚¬â€ highest variance cost component with no natural ceiling per conversation |
...
| Scaling behavior | **Linear to superlinear** Ã¢â‚¬â€ log volume scales with request count. Verbose logging (DEBUG level) or high-cardinality trace attributes can cause 10-50x volume increase. Tool-calling conversations generate multiple log entries per round. |
...
| Risk level | **Medium** Ã¢â‚¬â€ predictable at low volume but can spike with verbose config or traffic surges |
...
| Scaling behavior | **Sublinear initially, linear long-term** Ã¢â‚¬â€ audit trail and chat history grow with every operation. Without archival, database size grows indefinitely. SQLite VACUUM can reclaim space from deletions. |
...
| Action owner | Infrastructure lead |
| Risk level | **Low** Ã¢â‚¬â€ predictable growth, but uncapped audit trail could become significant over years |
...
| Scaling behavior | **Linear** Ã¢â‚¬â€ each connected user maintains one persistent WebSocket. Memory: ~50-100 KB per connection. Network: minimal for idle connections, increases with board mutation frequency. |
...
| Risk level | **Low** Ã¢â‚¬â€ negligible cost at single-node scale; becomes relevant at 500+ concurrent connections |
...
| Scaling behavior | **Step function** Ã¢â‚¬â€ cost jumps when exceeding free-tier minutes (2,000 min/month for free, 3,000 for Pro). Docker image storage grows with image count and tag retention. |
...
| Risk level | **Low** Ã¢â‚¬â€ predictable and within free-tier for most open-source projects |
...
| Scaling behavior | **Linear with external integration frequency** Ã¢â‚¬â€ each MCP tool call is an HTTP request. Write tools that produce proposals may trigger LLM downstream. Rate limited at 60 req/60s per API key. |
...
| Action owner | Product/backend lead |
| Risk level | **Low** Ã¢â‚¬â€ rate-limited and proposal-gated; indirect LLM cost is covered by Hotspot 1 |
