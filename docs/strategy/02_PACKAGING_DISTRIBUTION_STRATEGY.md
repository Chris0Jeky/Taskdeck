...
_(This is no longer "to be added" — `.github/workflows/release-desktop.yml` and `scripts/build-release.sh` **already exist and are wired** (see line 69). The workflow **auto-fires on any `v*` tag push** and on `workflow_dispatch`, building the matrix `win-x64`/`linux-x64`/`osx-x64`/`osx-arm64` self-contained exes and creating a GitHub Release with SHA256 checksums + a `#1123` post-publish smoke test. Under the archive pivot this is the **optional archival exe-release** path — cutting a `v0.1.0` tag auto-publishes that archival Release; the *broader* distribution (stores, download pages, marketing) below is parked.)_
