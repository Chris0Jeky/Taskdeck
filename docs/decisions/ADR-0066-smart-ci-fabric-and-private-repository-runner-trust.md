...
**Amendment (2026-09-03, maintainer decision packet SC1 — "confirm with private-Pro approval-boundary amendment").** All nine rulings are confirmed. A personal GitHub Pro private repository has no organization ruleset, required workflow or restricted runner group, so the approval boundary the pack assumed an organization would supply is the maintainer's own account: workflow runs for outside or agent-authored changes require the maintainer's approval in the repository's Actions policy, the base-ref `pull_request_target` control plane stays read-only with no head execution, and control-plane (T0/T2) and runner changes are approved only by the maintainer's review — never by an agent, a bot review, or an environment binding alone. CI-13 `#2337` step J.6 re-checks that policy in private mode; CI-14 `#2338` remains the path to an organization-supplied boundary. Ruled in the same packet and recorded in `OUTSTANDING_TASKS.md` §J: SC-3 a $10/month hard Actions ceiling (the setting is still the maintainer's to apply); SC-4 no pre-ruling on `strict`/`enforce_admins` before the observation window; SC-8 a private development repository plus a public release/source mirror (CI-16 `#2439`); SC-2 the bounded artifact deletion after a fresh dry run.
