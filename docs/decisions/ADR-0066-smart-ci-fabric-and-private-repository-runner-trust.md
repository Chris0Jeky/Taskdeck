...
- the required workflow runs the **whole** suite on `pull_request`, again on every `push` to `main` (a merge commit), and on `merge_group`; a green PR run has 17 jobs and a ~23-minute critical path;
- Windows runs as a second semantic platform (full backend unit, full API integration, full frontend lint/type/build/coverage after a launcher regression, plus the worktree-helper harness) — the API suite alone is ~16 minutes on Windows against ~7.5 on Ubuntu, and the delta is process startup, readiness polling, fixed delays and teardown in the MCP HTTP/process suites, not semantics;
- the Actions cache sits at the 10 GB cap (179 caches) and **31,942 artifacts** are on record;
