...
- **Status:** Accepted under delegation (2026-08-30) — the maintainer directed the programme in-session ("the repo will need to go private for v0.3.0 main release, and initially it should work with only my github pro account, without having to rely on teams/enterprise"); the agent pass ruled on the pack's open conditions and recorded every ruling on tracker CI-00 `#2324`, where one reply overturns any of them. **Confirmed by the maintainer on 2026-09-03** (decision packet SC1) with the *private-Pro approval-boundary amendment* recorded under *Acceptance conditions and delegated rulings*. The four actions only the maintainer can perform — repository visibility, spend ceiling, branch-protection/ruleset edits, runner registration — are **not** delegated and stay open in `OUTSTANDING_TASKS.md` §J and on CI-13 `#2337`.
- **Date:** 2026-08-30
...
Before the visibility change (CI-13 `#2337`, human gate): baseline recorded; planner in shadow with the recall report; gate observed then registered, with `strict: true` / `enforce_admins` decided on the observation evidence (SC-4, no pre-ruling 2026-09-03); landed verifier and direct-push escalation proven; actions SHA-pinned; storage under the allowance or a spend decision recorded; hosted-only rehearsal of R0/R2/R3/R4 PRs, a merge, a nightly no-change skip and a release dry-run; public-asset and review-integration billing decisions recorded; spend ceiling and alerts set. The maintainer changes visibility manually; runners are registered only afterwards.

**Amendment (2026-09-03, maintainer decision packet SC1 — "confirm with private-Pro approval-boundary amendment").** All nine rulings are confirmed. A personal GitHub Pro private repository has no organization ruleset, required workflow or restricted runner group, so the approval boundary the pack assumed an organization would supply is the maintainer's own account, and it is made of three things that actually hold on a personal account — not of GitHub's workflow-run approval setting, which only covers fork/outside-contributor runs and gates nothing for a branch pushed to this repository by an agent. (1) **Execution stays hosted-only** (ruling 4) until CI-04 proves the isolated runners, and even then a same-repository agent PR reaches a self-hosted runner only through the Decision 8 residual, which stays recorded and is closed by the CI-14 `#2338` organization path (ruleset-required workflow + workflow-restricted runner group), not by an Actions setting. (2) The base-ref `pull_request_target` control plane stays read-only with no head execution. (3) Control-plane (T0/T2) and runner changes merge only after the maintainer's review and the fresh-context review every workflow change receives — never on an agent's, a bot review's, or an environment binding's say-so. The fork/outside-collaborator run-approval policy is still re-checked at CI-13 `#2337` step J.6 for what it does cover. Ruled in the same packet and recorded in `OUTSTANDING_TASKS.md` §J: SC-3 a $10/month hard Actions ceiling (the setting is still the maintainer's to apply); SC-4 no pre-ruling on `strict`/`enforce_admins` before the observation window; SC-8 a private development repository plus a public release/source mirror (CI-16 `#2439`); SC-2 the bounded artifact deletion after a fresh dry run.
