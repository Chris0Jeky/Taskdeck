...
5. **Scoped exception to the no-new-backend-surface rule (#1269):** the Phase-2 transcript engine (LLM-backed triage strategy, Transcript entity, evidence spans, OpenAI-compatible provider, risk-tiered review prioritization and batch-confirm ergonomics) and the Phase-1 registration gate and beta feedback channel are authorized new surface. **ADR-0003 remains controlling:** every automation-originated board write still requires explicit approve, then explicit execute; no standing policy or confidence threshold may auto-apply it. Everything else stays under the rule.
