...
1. **Default mode flips to `paper`** (`paperThemeStore` fallback changes from `off` to `paper`). The storage key is bumped (`td.paper.mode` → `td.paper.mode.v2`) so browsers that stored `off` before this decision pick up the new default; an explicit user choice made after the flip is honored.
