...
| ~~MEDIUM~~ RESOLVED | ~~`style-src 'unsafe-inline'` in CSP~~ **RESOLVED** (SEC-29 `#855`/`#945` partial + `#855`/`#988` complete): API CSP no longer has `'unsafe-inline'` in `style-src`; all 22 `:style` bindings across 14 Vue files migrated to CSS custom properties/v-bind()/utility classes; `'unsafe-inline'` dropped from `style-src` in reverse-proxy CSP | No longer allows inline style injection |
