...
| ~~MEDIUM~~ PARTIAL | ~~`style-src 'unsafe-inline'` in CSP~~ Removed from API CSP; reverse proxy adds `style-src-elem 'self'` to block inline `<style>` in modern browsers. `'unsafe-inline'` retained in `style-src` as fallback for Vue reactive `:style` bindings (SEC-29 `#855`). Follow-up issue to migrate remaining dynamic bindings. | Allows inline style injection |
