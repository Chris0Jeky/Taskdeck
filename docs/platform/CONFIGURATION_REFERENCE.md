...
| `Llm:OpenAi:Model` | `string` | `gpt-5.6-luna` | Model identifier sent in chat requests. Reasoning-family models (`gpt-5*`, `o1*`, `o3*`, `o4*`) are detected by `OpenAiLlmProvider.IsReasoningModel`; for them the outbound payload omits `temperature`, which those models reject. Every model receives `max_completion_tokens` (not the legacy `max_tokens`). | No |
