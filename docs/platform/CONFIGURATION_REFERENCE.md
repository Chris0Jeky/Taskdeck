...
runaway, already-abandoned ones) may run at once, and `ExtractionMaxDecodedBytes`
caps the total decoded output a single extraction may materialize, so a
decompression-bomb PDF cannot exhaust memory. Together they bound worst-case
in-process extraction work at `ExtractionMaxConcurrency × ExtractionMaxDecodedBytes`
(see ADR-0047).
...
| `Artefacts:ExtractionMaxConcurrency` | `int` | `2` | Maximum number of extraction parse workers permitted to run at once, enforced by a process-wide permit gate (`ArtefactExtractionGate`). A parser-bomb PDF that never observes cancellation keeps a thread-pool thread at full CPU until PdfPig's synchronous parse finishes — only the request is bounded by `ExtractionTimeoutSeconds`, not the abandoned thread — and the permit is held for that thread's full lifetime, not just the request's. Once the permits are exhausted (including by abandoned bombs still spinning), further extractions are rejected pre-parse with `TooManyRequests` and write no extraction-history row, so box-wide CPU burn is capped at this many spinning threads. Rejection, not queueing: a queue in front of held permits would be a second unbounded backlog. Bounded to `[1, 64]`. Environment variable: `Artefacts__ExtractionMaxConcurrency`. | No |
| `Artefacts:ExtractionMaxDecodedBytes` | `long` | `134217728` (128 MiB) | Cumulative cap, in bytes, on the decoded output a single extraction may materialize across every compressed stream in the artefact. The input byte cap bounds the compressed size, but a decompression-bomb stream (a few KiB of FlateDecode that inflates to gigabytes) stays under it while exhausting memory during the decode. A bounded filter provider (`BoundedFilterProvider`, injected into PdfPig via `ParsingOptions.FilterProvider`) counts decoded bytes as it goes — with a streaming pre-pass for FlateDecode that aborts before the real decode materializes anything — and stops the moment this cumulative ceiling is crossed, recording a content-free `decoded-size-limit` warning row rather than an out-of-memory crash. Floored at 65536 (64 KiB) so an operator cannot configure a ceiling below a single decode buffer; the default comfortably clears any legitimate ≤10 MiB PDF. Environment variable: `Artefacts__ExtractionMaxDecodedBytes`. | No |
