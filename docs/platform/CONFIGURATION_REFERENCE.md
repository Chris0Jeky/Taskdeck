...
| `RateLimiting:McpAuthenticationPerIp:WindowSeconds` | `int` | `60` (class default) | same | Window length in seconds for the MCP authentication-failure budget. Also used as the `Retry-After` value on a pre-check 429. | No |
