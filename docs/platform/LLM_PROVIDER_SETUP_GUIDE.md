...
- in the packaged Windows desktop build only, retired provider configuration inherited from the PROCESS ENVIRONMENT (retired `Llm__Gemini__*` children and a retired `Llm__Provider` selector) is dropped from configuration before selection so a leftover Gemini-era profile still starts on the packaged default, announced once as `TASKDECK_DESKTOP_WARNING code=retired_provider_configuration_ignored` and in the in-app provider status; retired settings in Taskdeck's own `appsettings.json` / `appsettings.local.json`, and every non-desktop host, keep the fail-closed behaviour above (#2233)
