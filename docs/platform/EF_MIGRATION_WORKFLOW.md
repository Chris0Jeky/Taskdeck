...
- **Per-process `Database.Migrate()` race (open)**: each host still calls `Database.Migrate()` on startup with no cross-process lock, so two processes booting simultaneously can race to apply migrations. `busy_timeout` mitigates the contention (the loser waits rather than erroring), but explicit single-owner serialization (advisory/file lock) is tracked as a follow-up — see issue #1164 (split from the now-closed #1130).
