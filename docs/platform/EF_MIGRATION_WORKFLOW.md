...
- **Local concurrency (WAL + `busy_timeout`)**: Taskdeck is local-first and the API UI, the MCP server, and the CLI may all open the **same** SQLite file. `SqlitePragmaConnectionInterceptor` (registered in `Taskdeck.Infrastructure.DependencyInjection`) sets `PRAGMA journal_mode=WAL` and `PRAGMA busy_timeout` (default 5000ms, see `Database:BusyTimeoutMilliseconds`) on every connection open. WAL lets many readers run alongside one writer; `busy_timeout` makes a contended writer wait for the lock instead of failing immediately with `SQLITE_BUSY` ("database is locked"). WAL is persisted in the database header (`*.db-wal` / `*.db-shm` side files appear next to the DB and are git-ignored).
- **Per-process `Database.Migrate()` race (open)**: each host still calls `Database.Migrate()` on startup with no cross-process lock, so two processes booting simultaneously can race to apply migrations. `busy_timeout` mitigates the contention (the loser waits rather than erroring), but explicit single-owner serialization (advisory/file lock) is tracked as a follow-up — see issue #1130's remaining acceptance criterion.
