...
| Decompression bomb, oversized image, extreme pixel dimensions, PDF page/object bomb, or huge extracted text | GEN-01 `#1315` shipped bounded streaming upload plus allowed-type/signature checks, and GEN-02 `#1316` shipped bounded local text extraction with cancellation and typed failures. Those in-process PDF bounds do not contain PdfPig xref-stream decompression: cancellation can abandon the awaited parse while the parser continues consuming memory. Vision/OCR decoded-pixel budgets are also not yet shipped. | ADR-0048 / `#1429` remains the hard gate for a memory-capped extraction worker (Windows Job Object or container cgroup) before PDF extraction is wired into any dependent artefact-triage or release path. GEN-03 `#1317` separately adds conservative decoded-pixel/provider budgets before consent-gated multimodal extraction. | Until `#1429` lands, a crafted PDF can exhaust host memory despite upload, text, or timeout limits. After process containment, a payload inside every individual limit can still consume meaningful local CPU or provider quota. |
