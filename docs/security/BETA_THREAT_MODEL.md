...
| Account enumeration via login timing | `LoginAsync` pays one dummy BCrypt verify against a fixed non-credential hash when `ResolveLoginCandidatesAsync` yields no candidate (`AuthenticationService.cs:56-63`), so absent and present identifiers cost the same; error code and message were already unified. The fixed-window `AuthPerIp` limit additionally slows probes | **shipped** — `#3426`. Residual: a correct password on an inactive account still returns a distinct `inactive` message (`:84-85`); unifying it is a user-visible behavior decision, intentionally left for a follow-up |
...
| Runaway spend | `LlmQuota:RequestsPerHour` 60, `TokensPerDay` 100000, optional `GlobalBudgetCeilingTokens` ([configuration reference](../platform/CONFIGURATION_REFERENCE.md#llmquota)) | **partial** — the ceiling defaults to unlimited **and is not a whole-instance cap**: it is evaluated **per `LlmSurface`**. The check reads `GetTotalTokensAsync(null, surface, …)` (`Application/Services/LlmQuotaService.cs:88-91`) and the reservation SQL filters `WHERE Surface = {surfaceValue}` (`Infrastructure/Repositories/LlmUsageRecordRepository.cs:149,183`), so every `LlmSurface` that actually reaches the quota path gets its own full ceiling. **Only two surfaces reach the quota path:** `Chat` via `Application/Services/ChatService.cs:354,900` (reserve; e.g. `:442` commit) and `Application/Services/WorkspaceObservationService.cs:39` (reserve, `:122` commit; shares the `Chat` quota and kill switch, `:12,28`), and `CaptureTriage` via `Application/Services/LlmCaptureTriageExtractor.cs:289-292` (reserve, `:341-344` commit) — the three `ILlmQuotaService` reservation/commit production callers, still two distinct surfaces. The enum's third member, `Worker` (`Domain/Enums/LlmSurface.cs:7-11`), appears only in `ILlmKillSwitchService.IsKilledAsync` checks in `Application/Services/AudioTranscriptionService.cs:16,22,47`; that service takes a kill switch but no `ILlmQuotaService` (`:10-11`), so it never reserves or commits quota. When a positive ceiling is configured, the nominal admission allowance can reach **2 × that per-surface value** across those two surfaces; real billed usage can exceed the reservation estimates. Reserving under `Worker` would add a third separate allowance. Token budgets gate the reservation **estimate**, not the billed call: the estimate (default 2000, minimum 1) is replaced by provider-reported tokens when available or committed as an estimate when usage is unknown, so provider billing can differ from both admission and settlement counts. Concurrent-admission evidence is the row below — same-process SQLite only, not exact spend |
| Concurrent callers overshoot the budget | Atomic single-statement reservation: conditional `INSERT … SELECT … WHERE` evaluated under SQLite's single-writer lock, holding `ReservationEstimatedTokens` (default 2000, minimum 1) per in-flight call with a TTL sweep (`Infrastructure/Repositories/LlmUsageRecordRepository.cs:128-163`; `Application/Services/LlmQuotaSettings.cs:33-42`) | **partial — bounded same-process admission; spend still not exact.** The restored `#1435` tests pin one host and race independent scopes and connections against one database: request- and token-boundary bursts admit exactly one caller and persist exactly one reservation row, and fresh-file tests repeat that on real-migrated WAL files with setup connections closed first ([quota evidence note](../analysis/2026-09-20-quota-cold-start-evidence.md); `STATUS.md` section "Quota concurrency tests share one physical database (#1435)"). The earlier multi-database diagnostic did not establish a single-database WAL failure, and no production reservation SQL, startup, or schema changed. Not claimed: cross-process or multi-host atomicity, provider-side limits, or exact spend — admission counts the estimate, while commit records the supplied settlement value (authoritative usage when available, an estimate otherwise) |
...
5. **Quota admission is bounded same-process; spend is still not exact** (`#1435`). Concurrent reservation bursts against one SQLite database admit exactly one caller per boundary in the restored tests (one host, independent connections, fresh-file WAL coverage), and the existing settlement guarantees stand — commit records the supplied settlement value (authoritative usage when available, otherwise an estimate) even if the reservation was TTL-swept mid-call, known unbilled failure paths release their reservations, and stale reservations are swept. But budgets gate the reservation estimate while commits may record only estimated usage, nominal admission allowances apply per surface (up to 2 × today across `Chat` and `CaptureTriage`), and cross-process, multi-host, and provider-side spend are unqualified. Keep a provider-side hard cap (§6, item 7).
   See the [quota evidence note](../analysis/2026-09-20-quota-cold-start-evidence.md) and `STATUS.md` ("Quota concurrency tests share one physical database (#1435)").
