...
| CLI (standalone `Taskdeck.Cli`) | `backend/src/Taskdeck.Cli/Program.cs:20,125-127` (top-level `try` / `catch (Exception exception)` delegating to `CliUnexpectedFailure.Handle`); boundary `backend/src/Taskdeck.Cli/Commands/CliUnexpectedFailure.cs:27,46` | stderr gets one stable generic line (`CliUnexpectedFailure.Message`, aliased to `GenericUnexpectedFailureMessage`) plus a bounded startup-trace correlation reference; no raw message, stack trace or path is printed. Deliberate `DomainException` / usage / pre-migration-backup failures keep their authored messages | `GenericUnexpectedFailureMessage` via `CliUnexpectedFailure.Handle` | safe — closed by `#2466` | `backend/tests/Taskdeck.Cli.Tests/CliUnexpectedErrorSafetyTests.cs` (11 cases, incl. `Handle_WithoutTrace_PrintsOnlyTheStableGenericLine` `:36`, `Handle_WithTrace_WritesTheFullExceptionExactlyOnceToTheProtectedSink` `:85`, `UnexpectedFailureMessage_IsTheCanonicalRedactorConstant` `:113`, `RealCli_WhenStartupThrows_ExitsWithFailureAndSafeStdErr` `:233`) |
...
- **R7 — CLOSED by `#2466` (merge `57df469e2`): the standalone CLI has a top-level boundary.**
  `Program.cs:20,125-127` wraps the whole dispatch in `try` / `catch (Exception exception)` and
  ...
  exception in the protected trace sink only. Pinned by
  `backend/tests/Taskdeck.Cli.Tests/CliUnexpectedErrorSafetyTests.cs` (11 cases). The companion
  failure file shipped harness-only; `#2468` tracks making the diagnostic sink always-on.
