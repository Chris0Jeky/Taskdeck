...
| MCP HTTP telemetry | `Mcp/McpTelemetryMiddleware.cs:112` | Nothing reaches the client; the broad catch records the exception type and rethrows | logging sink | safe | covered by `McpTelemetryMiddlewareTests` |
...
- **Rule 2 (`unknown-exception-text`)** — in `backend/src/Taskdeck.Api/Mcp/*.cs` **and** in the files
  ...
  catch in `McpTelemetryMiddleware.cs:112` is exempt on its merits — it logs only, and reports the
  exception *type* name rather than its message — not by
  allowlist.
