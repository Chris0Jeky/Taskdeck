...
2. **External contributions paused; reopening requires a relicensing-capable inbound instrument ([ADR-0067](decisions/ADR-0067-open-core-commercial-model-and-inbound-rights.md), 2026-09-03).** Contributions accepted before 2026-08-24 came in under GPL-3.0-only with no blanket relicensing assignment collected (inbound equals outbound); none from outside the maintainer were ever accepted. Since 2026-08-24 external code contributions are paused, and they reopen only behind a contributor licence agreement or contribution grant that preserves relicensing flexibility. By explicit maintainer decision on 2026-08-23, `Signed-off-by:` trailers are optional and no DCO check runs; `#2019` is an inert future-restoration tracker. Commercial code may live in explicitly separately licensed modules (the reserved `ee/` pattern).
...
- **v0.3 M2 (re-scoped 2026-08-30, RC deck q-3 B):** `#2090` widths/collapse/titles-only stays in v0.3 (PR #2165 merged; titles-only shipped by PR #2666); `#2089` capture links/ledger/lifecycle and `#2087` minimal types/parent moved to v0.4. ADR-0060 is owned only by `#2084`; ADR-0061 decisions by `#1772`; ADR-0062 decisions by `#2091`.
...
Accepted 2026-07-13 after the maintainer's twin-app evaluation (decision: extend the single app, defer the twin behind the GEN-12 evidence gate — **that gate closed 2026-09-03: stay one application**, `#1326` closed under the decision packet; reopening it takes a new decision issue plus an ADR-0046 amendment). The GEN-00 `#1327` tracker and each lane's technical dependencies remain binding. **Strictly subordinate to the v0.1 ship gate** — lane **G-A** is parallel-safe, lane **G-B** follows REVIVAL-08 M1 (`#1312`), and lane **G-C** follows Phase 1.
...
| Twin-app decision gate (strategy; default **no**) — **closed 2026-09-03, stay one application** | GEN-12 `#1326` (closed) | ruled |
...
The GEN-11 friends-family channel (`#1325`) adds a second signal stream — non-technical activation and retention — reviewed at the same checkpoint; the GEN-12 (`#1326`) twin-app gate it once fed closed 2026-09-03 (*stay one application*), so the stream now informs the single app's generalist reach, not a twin decision.
