...
2. **No CLA collected to date; DCO enforcement paused; reopening requires an inbound instrument (ADR-0067).** Contributions are accepted for inclusion under GPL-3.0-only and no blanket relicensing assignment is collected. By explicit maintainer decision on 2026-08-23, `Signed-off-by:` trailers are optional and no DCO check runs; `#2019` is an inert future-restoration tracker. Commercial code may live in explicitly separately licensed modules (the reserved `ee/` pattern). *Amended 2026-09-03 ([ADR-0067](decisions/ADR-0067-open-core-commercial-model-and-inbound-rights.md)): external contributions stay paused and reopen only behind a relicensing-capable CLA or contribution grant; the inbound-equals-outbound sentence describes contributions accepted before 2026-08-24.*
