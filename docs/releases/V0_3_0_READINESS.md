...
Last Updated: 2026-09-04 (measured against `main` `330ccb4de`, 22:50Z)
...
The five clauses are `docs/REVIVAL_PLAN.md` §3, the v0.3 row. Clauses 2 and 4, the section 3 human-gate table and the section 5 label split re-measured 2026-09-04 at 22:50Z against `main` `330ccb4de`; the section 2 clause-5 chain carries its earlier 2026-09-04 measurement and clauses 1 and 3 their 2026-09-03 one.
...
| 4 | `main` green | **Not red, and not yet proven at the tip.** The last completed `ci-required` run on `main` is `33886539482` at `61e94f672` (success, 2026-09-04 15:21Z). Every push run since (`df1559fd8`, `46fb41d53`, `7155f1042`, `ea3e39e7d`, `0886b6c42`: five merges in 35 minutes) was cancelled by the workflow's own concurrency group (`cancel-in-progress: true` with no branch condition) as the next merge landed, not by a failing job; the `0886b6c42` run had 13 of 17 jobs green when cancelled, with `E2E Smoke` and `Secret Scan` skipped by design and only the two Windows legs unfinished. The tip run at `330ccb4de` (`33926429510`) was in progress at 22:50Z. Every merged head carried its own green required run, so nothing merged unproven; the missing evidence is the combined tip, tracked as `#2582` | Three open intermittent reds, section 2; `#2582` so a merge wave stops erasing the tip's run |
