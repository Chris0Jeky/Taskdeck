...
In demo mode: login is the "Enter Demo" path, review/chat/card metadata read from in-bundle fixtures, and SignalR is not started. Mutations are view-only or in-memory only; they are not a production backend.
