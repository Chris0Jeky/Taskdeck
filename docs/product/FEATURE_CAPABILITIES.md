...
| --- | --- | --- |
| Card archive/restore | Hide retained work from active surfaces; restore it to its original column subject to WIP. | Legacy archive proposals still mean Block. Active-card WIP and observation follow-ups are merged; restore preview now counts the occupancy the same proposal's earlier operations create or free (#2926). The conflict-warning projection still omits lifecycle effects. |
