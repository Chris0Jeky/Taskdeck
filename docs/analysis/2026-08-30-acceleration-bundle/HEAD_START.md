...
| --- | --- | --- | --- | --- |
| `#2256` CF-02 | "enums, mapping, columns exist in the scaffold; remaining work is intake/API stamping" | Live, not scaffold. `producerOverride` / `producedByPrincipalId` exist on `CaptureIntakeService` with zero callers, so producer is derived from client `source` | adapt | server-stamp producer/principal from the authenticated principal, fail closed |
| `#2257` CF-03 | state machine first; job/run tables absent | Tables absent is right; the live comment makes the canonical policy-snapshot digest the first freeze; candidate forks the shipped `ProcessingJobState`; `LlmUsageRecord` has no run link and no price/currency | adapt | policy snapshot record + canonical serializer + digest tests |
...
| `#2259` CF-05 | adapter migration; delete request-type SQL routing | The real blocker is C#: `CaptureTriageService` gates LLM triage on `IsTranscriptSource`; the SQL predicates are also used by the CF-01 backfill store and must stay | adapt | move the eligibility gate off `IsTranscriptSource` |
...
| `#2087` item types | ready after contract freeze; depth 3; `work_parent_*` codes | ADR-0060 accepted; nothing shipped; depth convention unruled; snake_case codes map to 500 | adapt | `WorkItemType` + `Card.ParentCardId` + additive migration + pure validator |
...
| `#2093` participants | blocked by the merged `#2240`; integer-minute estimates | `#2240` is open and forked, not merged; the estimate has no assignment dependency; unit unruled | adapt | nullable bounded estimate on the card path |
...
| `#2259` CF-05 | move the triage eligibility gate off `IsTranscriptSource` | One predicate; no CF-03/CF-06 dependency |
