...
| --- | --- | --- | --- | --- |
| `CF03-1-policy-snapshot` | Freeze the minimal `ProcessingPolicySnapshot` (egress class, allowed processors, diarisation/alignment flags, deadline, cost ceiling) **plus its canonical digest** — pinned field order, pinned number/date formatting, kebab-case enum spelling matching `StrictKebabCaseEnumConverterFactory` | Draft contract inputs landed by PR `#2371` | contract-only | **Yes, after reconciling the draft inputs with the live issue and ADR.** This rebased branch includes the schema, fixture and checker, and the checker passes; the artifacts remain proposed inputs rather than an accepted contract. The slice remains a pure record + canonicalizer + tests touching no shipped writer |
