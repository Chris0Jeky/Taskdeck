...
Cross-pack: vocabulary is clean everywhere ("Controlled" appears nowhere; Private/Balanced/Strict/Expert, Flow/Guided/Control and the six ADR-0057 presets are used correctly). The generic `EDGE_CASES` / `TEST_PLAN` boilerplate is identical across all seven packs and was kept only as a floor. ADR-0057 spells the fifth authority preset "Autonomous/Expert" where ADR-0065 says "Autonomous"; use *Autonomous* so *Expert* stays a processing term.
