...
- [ ] **#1293 / PR #1295** — FLOOR consolidation: thin deny-floor overlay (`pre_tool_use.py`). **PR OPEN, NOT merged (T4/floor-class, human-merge).** gemini bot findings fixed (`916ea0b8`); a 6-lens adversarial sweep found **18 issues** (1 CRITICAL) — see the PR triage comment. **MUST-FIX before merge:** `rm -Rf ~`/root recursion (gate on recursion alone + deny roots); DROP defeated by `;`/`|` inside quoted SQL (quote-aware splitter); `.pem`/`id_rsa`/`credential` deletable via `Remove-Item`/`cp`; quoted-secret-filename bypass; PowerShell `-r` `-Recurse` abbreviation. Plus SHOULD-fix (`git checkout .`/`git restore <path>` work-loss holes, `npm publish --dry-run`, `dotnet ef --verbose … drop`, fail-closed test, `token_a.txt` runbook false-positive). **Side-findings to file vs `source/agent-harness`:** dispatch.py `-Rf` case-sensitivity + `bash -c` wrapper blindness.
...
## Changelog
- 2026-07-07: **#1293 FLOOR consolidation — PR #1295 opened (thin overlay), NOT merged.** `pre_tool_use.py` rewritten as a bypass-proof thin complement to the global floor; `smoke_test.py` + `tier.json` reconciled. gemini bot review fixed. Independent 6-lens adversarial sweep surfaced 18 findings (1 CRITICAL `rm -Rf ~` root gap) — triaged on the PR; remediation deferred to next session. Do not merge until MUST-FIX items land + green CI + re-review.
