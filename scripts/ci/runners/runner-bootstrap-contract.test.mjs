...
test('cleanup trusts only immutable fixed policy and removes children, never roots', () => {
  ...
  assert.match(windowsCleanup, /Assert-NoReparseDescendants -Root \$Root/)
  assert.match(windowsCleanup, /Stack\[string\]/)
  assert.match(windowsCleanup, /child_reparse/)
  assert.match(windowsCleanup, /acl_runner_control/)
  assert.match(windowsBootstrap, /acl_runner_control/)
