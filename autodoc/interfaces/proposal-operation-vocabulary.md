...
- `estimatedEffortMinutes` is optional on card `create` and `update`: a JSON integer from `0` through `1000000`, with `0` a known zero estimate and `null`/omission unknown on creation or unchanged on update. Strings, booleans, fractions and out-of-range numbers are rejected. An update with `clearEstimatedEffort: true` explicitly clears to unknown; a non-null estimate and true clear flag conflict. The clear flag must be a JSON boolean and is supported only on update. An estimate-null/clear-false payload alone is not an update. Estimate fields are rejected on all other operations.
- An estimate set or clear on an existing card requires `expectedUpdatedAt` from its displayed state. Every such operation in a proposal pins the same initial persisted timestamp; preview and apply validate all pins before writing. A card created earlier in the proposal has no initial persisted timestamp. Apply uses the current tracked card version after earlier approved operations while preserving the persistence concurrency token and transaction rollback. Preview carries each estimate into the next operation, showing hours/minutes (`1h 30m`), unknown and `0m` distinctly; this is planned effort, not logged time.
- Chat tools expose `estimated_effort_minutes` on `propose_create_card` and `propose_update_card`; update also accepts `clear_estimated_effort` and the exact `expected_updated_at` returned by `get_card_details`. The tools preserve JSON types and the caller's timestamp for shared validation, create only reviewable proposals, and never write card state directly.
...
- Preview and Apply validate the same effective revision payload with `ProposalOperationContractValidator` and use the same field parsers. Invalid/conflicting dates or estimates, malformed labels, stale estimate pins, or scope redirects cannot produce an approval preview.
