...
| Local bootstrap configuration | `Api/FirstRun/FirstRunBootstrapper.cs`, `Application/Bootstrap/BootstrapFileLock.cs`, `BootstrapFileSecurity.cs`, `Cli/CliFirstRunBootstrapper.cs`, API `Program.cs` | Resolve one absolute local-config path per host; desktop Production uses durable app-data and imports an absent executable-local legacy file once; owner-only persistence and existing-database/no-key ambiguity fail closed; headless Production never auto-generates a connector key | `dotnet test backend/Taskdeck.sln -c Release -m:1 --filter "FullyQualifiedName~LocalConfigPathMigrationTests|FullyQualifiedName~FirstRunBootstrapperTests|FullyQualifiedName~CliFirstRunBootstrapTests"` |
...
| Harness / CI / docs | `.claude/`, `scripts/agent_hooks/` (deny floor + ledger), `.github/workflows/` (`ci-required.yml` = the gate), `scripts/check-*.mjs` | `ci-required` is the sole merge gate; docs-governance `Last Updated: YYYY-MM-DD` exact line; hook `smoke_test.py` green; deny-floor changes are T4-class; native PowerShell-tool interception remains [#1497](https://github.com/Chris0Jeky/Taskdeck/issues/1497) | Native-Windows-hosted configured-handler smoke with Bash payloads: `py -3 -B scripts/agent_hooks/smoke_test.py`; POSIX direct utilities only (the configured handlers require PowerShell); then docs gates (see `scripts/agent_hooks/CLAUDE.md`) |
