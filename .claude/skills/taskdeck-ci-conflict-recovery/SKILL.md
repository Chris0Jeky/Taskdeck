...
- Prefer merge over rebase when conflict resolution starts stalling; replace `BRANCH_NAME` with the source ref in `git merge --no-gpg-sign BRANCH_NAME`. After a conflict, stage the resolution and finish with `git commit --no-gpg-sign --no-edit` instead of `git merge --continue`.
