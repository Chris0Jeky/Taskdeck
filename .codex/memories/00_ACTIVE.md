...
**Direction (2026-07-10 revival pivot, ADR-0044):** Taskdeck is being revived and shipped as a **free open beta** — the local-first, review-first action-item engine (transcripts/notes in → evidence-linked proposals out → human-approved board apply), with the write-gated MCP server as the developer-facing second act. Active sequencing is `docs/REVIVAL_PLAN.md` (REVIVAL-00..14, `#1297`–`#1311`; `#1311` is the tracker), and the v0.1 ship gate has top priority. The 2026-06-13 archive pivot is **superseded** and retained only as the traction-checkpoint fallback. Work not on the ratified wave list is not taken. The project thesis remains unchanged:
