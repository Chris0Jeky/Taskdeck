...
- **BREAKING: none — proposal provenance records the producer triple.** Migration
  ...
  visible consequence for pre-upgrade rows: `GET /api/automation/proposals/{id}/provenance/metadata`
  ...
  because `ModelId` is also where the origin labels `chat-tools`, `manual` and `queue` (or `unknown`
  for an out-of-range source type) are stored and an origin label rendered as a model name would be
  a false producer claim. The Paper
...
- **Account deletion refuses while you own boards outright, and removes MFA/API-key credentials (#3425, #3400).** The sole-owner guard scanned only BoardAccess rows, but board creation records ownership in Boards.OwnerId — so creators could delete their accounts and orphan boards permanently. Deletion now fails closed naming the first owned board; delete your boards first (no ownership-transfer flow exists yet — see #3424). Successful deletion also deletes MFA credentials, clears the MFA flag, and deletes API keys; the result reports MfaCredentialsDeleted and ApiKeysDeleted counts. Automated deletions for board owners now receive InvalidOperation instead of orphaning boards.
