...
public sealed class WorkspaceAttentionService(IWorkspaceAttentionRepository repository, WorkspaceInsightService insights, TimeProvider clock)
{
    ...
    {
        if (dto.UpdateWindow && dto.Window is not null && !dto.Window.IsValid())
            return Result.Failure<WorkspaceAttentionDto>(ErrorCodes.ValidationError,
                "Choose a valid IANA time zone, at least one day and different start/end times.");
        var row = await repository.GetAsync(user, ct);
        ...
        var current = row.ReadAttention();
        var next = current with { Enabled = dto.Enabled, Window = dto.UpdateWindow ? dto.Window : current.Window };
    ...
    public async Task<Result<WorkspaceAttentionReminderDto?>> ClaimAsync(Guid user, Guid boardId, CancellationToken ct)
    {
        ...
        var next = candidate is null ? null : state.Claim(candidate.Id, clock.GetUtcNow());
    ...
    private static WorkspaceAttentionDto Map(WorkspaceAttention state, long revision) =>
        new(state.Enabled, revision, WorkspaceAttention.DailyLimit, (int)WorkspaceAttention.MinimumSpacing.TotalMinutes, state.Window);
