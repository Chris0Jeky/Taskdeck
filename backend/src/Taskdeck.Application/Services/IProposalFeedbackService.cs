...
public interface IProposalFeedbackService
{
    ...
    /// (proposal, user): a repeat is a no-op, except the first specific reason upgrades an
    /// earlier Unspecified one (first-specific-wins). Never changes the proposal's status.
