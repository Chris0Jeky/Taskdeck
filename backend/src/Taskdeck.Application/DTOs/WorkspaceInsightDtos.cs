...
public record PreserveMemorySourceItem(Guid Id, int Revision);
public record PreserveMemorySourcesDto(Guid BoardId, [Required, MinLength(1), MaxLength(50)] List<PreserveMemorySourceItem> Memories);
public record WorkspaceMemoryRevisionDto(string Title, string Text, string Status, int Revision, bool Archived, DateTimeOffset RecordedAt, Guid? AnswerSourceAssetId = null);
public record WorkspaceMemorySourcesDto(Guid CaptureId, Guid AnswerAssetId, Guid? EvidenceAssetId);
public record WorkspaceMemoryDto(Guid Id, Guid BoardId, string Title, string Text, string OriginalText, string Status, bool Archived, int Revision, DateTimeOffset CreatedAt, IReadOnlyList<WorkspaceMemoryRevisionDto> History, string? OriginalEvidence, ThinkingAnswerSourceDto? ThinkingSource = null, WorkspaceMemorySourcesDto? Sources = null);
