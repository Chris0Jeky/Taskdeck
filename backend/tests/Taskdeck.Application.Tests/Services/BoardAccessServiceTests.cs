...
public class BoardAccessServiceTests
{
    ...
    [Fact]
    public async Task GrantAccessAsync_ShouldReturnForbidden_WhenAdminGrantsOwnerRole()
    {
        var owner = CreateUser("owner");
        var granter = CreateUser("granter");
        var targetUser = CreateUser("target");
        var board = new Board("Test Board", ownerId: owner.Id);
        var dto = new GrantAccessDto(board.Id, targetUser.Id, UserRole.Owner);
        ...
        result.IsSuccess.Should().BeFalse();
        result.ErrorCode.Should().Be(ErrorCodes.Forbidden);
        _boardAccessRepoMock.Verify(r => r.AddAsync(It.IsAny<BoardAccess>(), default), Times.Never);
        _unitOfWorkMock.Verify(u => u.SaveChangesAsync(default), Times.Never);
    }

    [Fact]
    public async Task GrantAccessAsync_ShouldSucceed_WhenOwnerGrantsOwnerRole()
    {
        var owner = CreateUser("owner");
        var targetUser = CreateUser("target");
        var board = new Board("Test Board", ownerId: owner.Id);
        ...
        var result = await _service.GrantAccessAsync(dto, owner.Id);
    ...
    [Fact]
    public async Task UpdateAccessAsync_ShouldReturnForbidden_WhenAdminUpdatesToOwnerRole()
    {
        var owner = CreateUser("owner");
        ...
        var board = new Board("Test Board", ownerId: owner.Id);
        var access = new BoardAccess(board.Id, targetUser.Id, UserRole.Viewer, owner.Id);
        var dto = new UpdateAccessDto(UserRole.Owner);
        ...
        result.IsSuccess.Should().BeFalse();
        result.ErrorCode.Should().Be(ErrorCodes.Forbidden);
        access.Role.Should().Be(UserRole.Viewer);
        _unitOfWorkMock.Verify(u => u.SaveChangesAsync(default), Times.Never);
    }

    [Fact]
    public async Task UpdateAccessAsync_ShouldSucceed_WhenOwnerUpdatesToOwnerRole()
    {
        var owner = CreateUser("owner");
        var targetUser = CreateUser("target");
        var board = new Board("Test Board", ownerId: owner.Id);
    ...
    [Fact]
    public async Task RevokeAccessAsync_ShouldEvictRevokedUserConnections()
    {
        var owner = CreateUser("owner");
        var targetUser = CreateUser("target");
        var board = new Board("Test Board", ownerId: owner.Id);
        ...
        result.IsSuccess.Should().BeTrue();
        evictorMock.Verify(e => e.EvictUserFromBoardAsync(board.Id, targetUser.Id, default), Times.Once);
    }

    [Fact]
    public async Task RevokeAccessAsync_ShouldNotEvict_WhenRevocationFails()
    {
        var owner = CreateUser("owner");
        var viewer = CreateUser("viewer");
        var targetUser = CreateUser("target");
        ...
        var result = await service.RevokeAccessAsync(board.Id, access.Id, viewer.Id);

        result.IsSuccess.Should().BeFalse();
        evictorMock.Verify(
            e => e.EvictUserFromBoardAsync(It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()),
            Times.Never);
    }
