...
- **v0.4 Hosted Open Beta + Work Model + Fabric Foundation:** the install-free hosted open beta reachable from anywhere (`#2243`), the work-model slices, opt-in analytics, and the behaviour-preserving Context Fabric foundation (ADR-0065, tracker `#2254`) — the durable capture aggregate, representations and evidence anchors, and the processor contracts that later releases build on.
