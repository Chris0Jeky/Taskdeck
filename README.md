...
For HTTP, create a key in **Settings → API Keys**, start the standalone command with the same `ConnectionStrings__DefaultConnection` as the web app, and configure the client from [mcp-http.example.json](mcp-http.example.json). The real route requires `Authorization: Bearer tdsk_...`; missing, invalid, expired, or revoked keys receive `401`, and `/` is not an MCP endpoint. Requests are rate-limited independently by the key's opaque ID.

The standalone server binds only to `127.0.0.1` by default and replaces a blank or wildcard `AllowedHosts` value with the loopback allowlist. Keep bearer keys on loopback. If you deliberately use `--host` for a container, tunnel, or deployment, terminate TLS before the request reaches an untrusted network and set `AllowedHosts` to the exact public host names; `--host` does not relax host-header validation. Cross-origin browser MCP is not enabled. One-command packaging and scoped-key hardening remain planned for [REVIVAL-13](https://github.com/Chris0Jeky/Taskdeck/issues/1309).
