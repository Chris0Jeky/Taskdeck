...
2. **Recovery.** Implement the missing consistent-backup/decrypt-verification path identified by the existing guides. Restore into a fresh fixture with the separately held connector key. Health and login alone do not prove connector decryptability. A Render cron service cannot simply attach another service's persistent disk.
